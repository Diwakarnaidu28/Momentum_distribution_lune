...
\end{align}
Note that we were able to use $ \sum_\ell \hat{V}(\ell)^m < \infty $, since $ m \ge 2 $. Also, we were able to absorb an arbitrary constant $ C $ into $ C^m $ and in the last line, we used $ (\cN+5)^m \le C (\cN+1)^m $ and the definition~\eqref{eq:Xi} of $ \Xi $.\\
...
\end{align}
Summing up the bounds and using $\sum_{j=1}^{m-1} {{m}\choose j} \le C^m $ concludes the proof.
\end{proof}
...
\end{aligned}
\end{equation}
For the case $ j = 0 $, using Lemmas~\ref{lem:normsk} and~\ref{lem:pairest} yields for $ q \in (-L_\ell + \ell + \ell_1) \cap (-L_{\ell_1} + \ell + \ell_1) $ \todo{why state explicitly which $q$ in this case?}:
\begin{align}
	|\I_0(\ell, \ell_1)|
