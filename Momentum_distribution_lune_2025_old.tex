...
Then, for any sequence\todo{This is a slight abuse of notation. Shall we leave it like this?} $ k_{\F} \to \infty $ with $ N = N(k_{\F}) := |B_{k_{\F}}(0)| $, there exists a sequence of trial states $ (\Psi_{N(k_{\F})})_{k_{\F}} $ with $ \Psi_N  \in L^2(\TTT^3)^{\otimes_{\mathrm{a}} N} $ such that
\begin{itemize}
...
\end{equation}
\item There exists a constant $ C > 0 $, depending on $ \Vert \hat{V} \Vert_1 $, such that for all $ k_{\F} $ and $ q \in \Z^3 $, the momentum distribution in $ \Psi_N $ satisfies
...
\end{equation}
which imply the bound $ \norm{a_p}, \norm{a_p^*} \le 1 $. This notation allows us to conveniently write the Fermi sea state as $ \Psi_{\FS} = R \Omega $ where $ R: \cF \to \cF $ is the unitary particle--hole transformation defined by
...
\begin{align}
	...
	&\leq \Bigg( \sum\limits_{r \in L_{\ell_1}} 
		\sum\limits_{s \in L_\ell} \abs{K^j(\ell)_{q,s}}^2
		\sum\limits_{s_1 \in L_{\ell_1}} \abs{K(\ell_1)_{r,s_1}}^2
