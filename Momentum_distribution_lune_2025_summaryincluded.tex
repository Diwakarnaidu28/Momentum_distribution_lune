...
\footnote{2020 \textit{Mathematics Subject Classification}. 81V74, 82D20, 81Q05.}
...
\end{equation}
In this article we are interested in the momentum distribution of states $ \Psi $ which are energetically close to $ \Psi_{\GS} $. The momentum distribution is the expectation value of the operator representing the number of fermions with momentum $q \in \Zbb^3$, that is
\begin{align}
...
\begin{equation}
	n(q) \approx 1 - n^{\RPA}(q) \;, \quad
	n^{\RPA}(q)
	\coloneq \sum_{\ell \in \Z^3_*}\mathds{1}_{L_{\ell}}(q+\ell) \; \frac{1}{\pi}\int_0^\infty \frac{g_\ell (t^2-\lambda^2_{\ell,q+\ell}) (t^2 + \lambda^2_{\ell,q+\ell})^{-2}}{1 + 2g_\ell \sum_{p \in L_{\ell}}\lambda_{\ell,p} (t^2+\lambda^2_{\ell,p})^{-1}} \mathrm{d}t \;.
\end{equation}
...
\item In analogy to~\cite[Sect.~1.1]{BL25}, in $ n^{\RPA}(q) $~\eqref{eq:nqb}, we may approximate $ \lambda_{\ell,q} \approx 2 k_{\F} |\ell| |\hat{\ell} \cdot \hat{q}| $ with $ \hat{q} := \frac{q}{|q|} $, then set $ \mu := t (2 k_{\F} |\ell|) $ and take the continuum limits $ \sum_{p \in L_\ell} \approx \int_{L_\ell} \di p $ and $ \sum_\ell \approx \int \di \ell $. The result is
\begin{equation}
\begin{aligned}
	n^{\RPA}(q)
...
Next, due to the difference in the definition of $\mathcal{S}$, the commutation relation between $a^*_qa_q$ and $\mathcal{S}$ changes and it now fixes the momentum within the Fermi ball. The final expansion and the errors analysis of the many-body and the bosonization error follows exactly as the former case with the momenta beimg summed over these crescent sets. And we see that the bootstrap argument is independent of where $q$ lies, hence works in this latter case. And keeping these differences and following the proof gives us the result for all $q \in \Z^3$.
\end{enumerate}
\end{remarks}

In the introduction, we present a pedagogical review and the main result. In Section~\ref{sec:trialstate}, we show the construction of the trial state over which we find the momentum distribution. In Section~\ref{sec:extraction}, we derive the iterated Duhamel expansion for the momentum distribution and differentiate between the leading order, the errors and the tail term resulting from the iterations. In Sections~\ref{sec:prelim_bounds} and~\ref{subsec:manybody_estimates}, we do the many-body error analysis, where in Section~\ref{sec:prelim_bounds} we perform some preliminary error estimates which we use throughout in the later sections. In Section~\ref{subsec:manybody_estimates}, we bound the tail term of the expansion in Section~\ref{subsec:tailestimate} and then perform estimates on the bosonization errors in Section~\ref{subsec:bos_error} while retaining a bootstrap quantity. In Section \ref{sec:leading_order_analysis}, we recover the integral representation of the leading order term in Section~\ref{subsec:integralrep} and we find the relative order of the leading order term in Section~\ref{subsec:control_nqb}. In Section~\ref{sec:mainthmproof}, we finally prove the main result using analysis done in previous sections and show the errors are sub-leading using a bootstrap argument.
...
\end{aligned}
\end{equation}
leading to Proposition~\ref{prop:finexpan}, which is the main result of this section. Note that $ a_q \Omega = 0 $, from which we immediately conclude $ \langle \Omega, a_q^* a_q \Omega \rangle = 0 $. The multicommutators are computed using the CAR~\eqref{eq:CAR}, where we extract $ n^{\RPA}(q) $ by bosonization, similarly as in~\cite{BL25}. The term $ n_q^{\ex} $ then appears when normal ordering the bosonization errors, similar to the exchange contribution to the correlation energy appearing in~\cite{CHN23}. Since $ n_q^{\ex} \ll n^{\RPA}(q) $ (compare Lemmas~\ref{lem:estnqex} and~\ref{lem:nqb_bounds}), we will treat $ n_q^{\ex} $ as a bosonization error.
...
In Lemma~\ref{lem:nqb_integralrecovery}, we will see that the first term on the r.~h.~s. of~\eqref{eq:finexpan} converges to $ n^{\RPA}(q) $ as $ n \to \infty $. The other two terms are error terms which we bound below.
...
\begin{align}
	&|\I_j(\ell, \ell_1)| \nonumber\\
	&\leq \Bigg( \sum\limits_{s \in L_\ell} \abs{K^j(\ell)_{q,s}}^2
...
\begin{equation} \label{eq:nqb_integralrecovery}
	n^{\RPA}(q) = \half\sum\limits_{\ell\in \Z^3_*}\mathds{1}_{L_\ell}(q) \big( \cosh(2K(\ell)) - 1 \big)_{q,q} \;.
\end{equation}
\end{lemma}
