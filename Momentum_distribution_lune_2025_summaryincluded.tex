...
\end{itemize}
The main result ahead holds true for all $q \in \Z^3 = B_F \cup B_F^c$. The corresponding lemmas and propositions along with the main result are proven for $q \in B_F^c$. The strategy to prove the result for $q \in B_F$ is detailed in the remarks below.
...
For $q \in B_F$, the analysis and the proof proceeds exactly as for $q \in B_F^c$, apart from a few key differences. The first difference Bogoliubov kernel, where the sum of the indices run over lunes shifted inside the Fermi ball or crescent sets:
\begin{equation}
...
\subsection{Recovering the Integral Representation for $ n_q^{\b} $}\label{subsec:integralrep}
...
\begin{proof}
We focus on the case $ q \in B_{\F}^c $, as $ q \in B_{\F} $ is treated analogously. For the upper bound on $ n_q^{\b} $, we use the sum representation~\eqref{eq:nqb_integralrecovery}, where we expand the $ \cosh $ and use Lemma~\ref{lem:normsk}, as well as $ 2 \lambda_{\ell,q} = e(q) + e(q - \ell) \ge e(q) $
\begin{equation}
	n_q^{\b}
