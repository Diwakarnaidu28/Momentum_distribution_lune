...
In this section we review the trial state construction of~\cite{CHN23}. We introduce fermionic Fock space as
\begin{equation}
	\cF \coloneq \bigoplus_{N=0}^\infty L^2_{\mathrm{a}}(\T^{3N}) \;,
\end{equation}
with vacuum vector $ \Omega = (1,0,0,\ldots) \in \cF $.
To each momentum $ q \in \ZZZ^3 $, we assign a plane wave
\begin{equation}
...
\begin{equation} \label{eq:K}
	K(\ell) \coloneq - \half \log \Big( h(\ell)^{-\half}
		...
		h(\ell)^{-\half} \Big) \;.
\end{equation}
(Our exponent $ S $ corresponds to $ R^* \cK R $ in the notation of~\cite{CHN23} since we prefer to use a particle-hole transformation instead of  the Wick ordering.)
