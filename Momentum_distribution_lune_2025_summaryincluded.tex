...
\end{equation}
Moreover we have \todo{Move to Remark 1?}
\begin{equation}
\lvert n^{\RPA}(q)\rvert \le C k_{\F}^{-1} e(q)^{-1} \;.
\end{equation}
\end{itemize}
% The analogous statements hold for $q \in B_F$, confirming \cref{eq:inside}.
\end{theorem}
...
\item In Lemma~\ref{lem:nqb_bounds}, we show that the scaling $ n^{\RPA}(q) \sim k_{\F}^{-1} e(q)^{-1} $ is sharp, in the sense that there exist $ V $ and $ q $ \todo{Give precise statement in terms of a sequence.} for which $ n^{\RPA}(q) \ge c k_{\F}^{-1} e(q)^{-1} $. \todo{Make sure that $ n^{\RPA}(q) \ge c k_{\F}^{-1} e(q)^{-1} $ is true.}
...
\begin{equation} \label{eq:EQ1EQ2extension}
	...
	= \sum_{j=1}^3 E_{Q_1}^{m,j}(q) + \mathrm{h.c.} \;, \quad
	E_{Q_2}(\Theta^m_{K}(P^q)) 
	= \Bigg( \sum_{j=1}^{11} E_{Q_2}^{m,j}(q) \Bigg) + \mathrm{h.c.} + n^{\ex,m}(q) \;,
\end{equation}
\todo{maybe make $q$-dependence of the $E$-terms explicit}
with
\begin{align}
	E_{Q_1}^{m,1}(q)
...
% \begin{equation*}
% 	n^{\ex}(q)
% 	= \half \sum_{\substack{m=1\\m:\textnormal{ odd}}}^\infty \frac{1}{(m+1)!} n^{\ex,m}(q) \;.
% \end{equation*}
...
% We start with some bounds on norms of powers of the correlation structure $ K(\ell) $.
...
\end{align}
Further, there exists $ C > 0 $ depending on $ m $, such that
\begin{equation}
...
We now turn to bounding the two errors of the expansion in Proposition~\ref{prop:finexpan}, namely the bosonization error $ E_m $ and the expansion tail. \todo{STATUS NIELS TYPING}
...
For any symmetric operator $ A(\ell): \ell^2(L_\ell) \to \ell^2(L_\ell) $, $ \ell \in \Z^3_* $, and $\Theta^n_K$ the $ n $-fold anticommutator defined in \eqref{eq:Theta}, we have
\begin{equation}
...
To prove this proposition, we must control the error terms $ E^{m,1}_{Q_2}(q) $ through $ E^{m,11}_{Q_2} (q)$, and $ n^{\ex,m}(q) $.
...
\begin{proof}
The proof strategy is very similar to the one of Lemma~\ref{lem:EQ111}: We only focus on bounding $ E^{m,1}_{Q_2}(q) $, since $ E^{m,2}_{Q_2}(q) $ is controlled analogously.
As in~\eqref{eq:EQ1111}, we split the anticommutator in $ E^{m,1}_{Q_2}(q) $ using~\eqref{eq:q-q}:
...
\begin{proof}
Splitting the anticommutator in $ E^{m,4}_{Q_2}(q) $ via~\eqref{eq:q-q} gives
...
\begin{proof}
We only focus on $ E^{m,5}_{Q_2}(q) $ as the bounds for $ E^{m,6}_{Q_2}(q) $ and $ E^{m,7}_{Q_2}(q) $ are analogous. Splitting $ E^{m,5}_{Q_2}(q) $~\eqref{eq:expandedEQ2} via~\eqref{eq:q-q} gives
...
\begin{proof}
We focus on $ E^{m,8}_{Q_2}(q) $, since the proof for $ E^{m,9}_{Q_2}(q) $ is analogous.
Splitting again the anticommutator in $ E^{m,8}_{Q_2}(q) $~\eqref{eq:expandedEQ2} via~\eqref{eq:q-q} we get
...
\begin{proof}
We focus on bounding $ E^{m,10}_{Q_2}(q) $, since the proof for $ E^{m,11}_{Q_2}(q) $ is analogous.
Splitting the multi-anticommutator in $ E^{m,10}_{Q_2}(q) $~\eqref{eq:expandedEQ2} via~\eqref{eq:q-q} yields
...
\begin{proof}
We recall definition~\eqref{eq:nqexm} of $ n^{\ex,m}(q) $, expand the multi-anticommutator via~\eqref{eq:q-q} and apply Lemma~\ref{lem:normsk} and $ \sum_{0 \le j \le m} {{m}\choose j} = 2^m $:
\begin{equation}
\begin{aligned}
	|n^{\ex,m}(q)|
