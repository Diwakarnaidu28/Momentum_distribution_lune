...
\item In~\cite{CHN23,CHN24}, both a bosonized and an exchange contribution were extracted for the correlation energy. Here, in addition to the bosonization contribution $ n^{\RPA}(q) $, we also find an exchange contribution $ n^{\ex}(q) $ in Lemma~\ref{lem:normalordering_errors}, given by
...
\end{equation}
From Lemma~\ref{lem:estnqex}, it follows that $ n^{\ex}(q) \le C k_{\F}^{-2} e(q)^{-2} $ for $ \hat{V} \in \ell^1 $, so $ n^{\ex}(q) $ is subleading. However, for Coulomb potentials $ \hat{V}(k) \sim |k|^{-2} $, we expect $ n^{\ex}(q) \le C k_{\F}^{-1} e(q)^{-2} $, so the exchange term should become observable close to the Fermi surface.
...
\item In analogy to~\cite[Section~1.1]{BL25}, in $ n^{\RPA}(q) $, we may approximate $ \lambda_{\ell,q} \approx 2 k_{\F} |\ell| |\hat{\ell} \cdot \hat{q}| $ with $ \hat{q} := {q}/{|q|} $, then set $ \mu := t (2 k_{\F} |\ell|) $ and take the continuum limit $ \sum_{p \in L_\ell} \approx \int_{L_\ell} \di p $ and $ \sum_\ell \approx \int \di \ell $. The result is
\begin{equation}
\begin{aligned}
	n^{\RPA}(q)
...
\end{aligned}
\end{equation}
leading to Proposition~\ref{prop:finexpan}, which is the main result of this section. Note that $ a_q \Omega = 0 $, from which we immediately conclude $ \langle \Omega, a_q^* a_q \Omega \rangle = 0 $. The multicommutators are computed using the CAR~\eqref{eq:CAR}, where we extract $ n^{\RPA}(q) $ by bosonization, similarly as in~\cite{BL25}. The term $ n^{\ex}(q) $ then appears when normal ordering the bosonization errors, similar to the exchange contribution to the correlation energy appearing in~\cite{CHN23}. Since $ n^{\ex}(q) \ll n^{\RPA}(q) $ (compare Lemmas~\ref{lem:estnqex} and~\ref{lem:nqb_bounds}), we will treat $ n^{\ex}(q) $ as a bosonization error.
...
To facilitate bounding the many-body errors $ E_{Q_1} $ and $ E_{Q_2} $ in \eqref{eq:errKQ}, we normal-order them. The exchange contribution $ n^{\ex}(q) $ then naturally appears by summing up the $ \C $-valued terms.
...
In particular, the exchange contribution $ n^{\ex}(q) $ in~\eqref{eq:nqex} follows as
\begin{equation*}
	n^{\ex}(q)
