...
To facilitate estimating the error operators $ E_{Q_1} $ and $ E_{Q_2} $, we normal-order them. The exchange contribution $ n^{\ex}(q) $ appears in this process.
...
\begin{equation} \label{eq:EQ1EQ2extension}
	...
	= \Bigg( \sum_{j=1}^{11} E_{Q_2}^{m,j} \Bigg) + \mathrm{h.c.} + n^{\ex,m}(q) \;,
\end{equation}
...
To prove this proposition, we must control the error terms $ E^{m,1}_{Q_2} $ through $ E^{m,11}_{Q_2} $, and $ n^{\ex,m}(q) $.
