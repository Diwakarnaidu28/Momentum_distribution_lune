...
In the introduction, we present a pedagogical review and the main result. In the section \ref{sec:trialstate}, we show the construction of the trial state over which we find the momentum distribution. In the section \ref{sec:extraction}, we derive the iterated Duhamel expansion for the momentum distribution and differentiate between the leading order, the errors and the tail term resulting from the iterations. In sections \ref{sec:prelim_bounds} and \ref{subsec:manybody_estimates}, we do the many-body error analysis, where in the section \ref{sec:prelim_bounds} we perform some preliminary error estimates which we use throughout in the later sections. In section \ref{subsec:manybody_estimates}, we bound the tail term of the expansion in \ref{subsec:tailestimate} and then perform estimates on the bosonization errors in \ref{subsec:bos_error} while retaining a bootstrap quantity. In section \ref{sec:leading_order_analysis}, we recover the integral representation of the leading order term in \ref{subsec:integralrep} and we find the relative order of the leading order term in \ref{subsec:control_nqb}. In section \ref{sec:mainthmproof}, we finally prove the main result using analysis done in previous sections and show the errors are sub-leading using a bootstrap argument.
