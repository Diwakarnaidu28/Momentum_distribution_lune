...
\begin{aligned}
	...
	\leq C^m k_{\F}^{-1} \Xi^{\half} e(q)^{-1}
		\norm { (\NN+1)^{\frac 32} \xi_\lambda } \;. \label{eq:estEQ213_Coulomb}
\end{aligned}
\end{equation}
If $ \sum_{\ell \in \Z^3_*} \hat{V}(\ell) < \infty $, then we have the even stronger bound
\begin{equation}
...
\end{equation}
In case $ m = 1 $, if $ \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^2 |k|^\alpha < \infty $ with $ \alpha \in [0,2] $, we have
\begin{equation}
