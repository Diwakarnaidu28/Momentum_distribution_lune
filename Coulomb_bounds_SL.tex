...
\end{equation}
\end{lemma}
\textcolor{green!30!black}{Note that, as mentioned above, we expect $ \Xi \sim k_{\F}^{-1} $, so the ``Coulomb bound''~\eqref{eq:estEQ111_Coulomb} is of order $ \sim k_{\F}^{-\frac 32} $, while~\eqref{eq:estEQ1111} is even of order $ \sim k_{\F}^{-2+\varepsilon} $. Also, the case $ m = 1 $ never occurs for $ E_{Q_1} $-error terms, which slightly simplifies the bounds, since $ \sum_\ell \hat{V}(\ell)^m < \infty $ is always true.}
