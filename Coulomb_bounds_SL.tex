...
Let $ \sum_{\ell \in \Z_3^*} \hat{V}(\ell)^2 < \infty $. For $\xi_\lambda = e^{-\lambda S} \Omega$, there exists $ C > 0 $ such that for all $ \lambda \in [0,1] $, $ m \in \NNN $, and $ q \in B_{\F}^c $,
...
\begin{align}
	...
	&\leq k_{\F}^{-\half} \Bigg( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \Bigg)^{\half}
		\Bigg( \sum_{r,\ell,\ell_1,s_1' \in \Z^3_*}
		\norm{ a_{-s_1'+\ell_1} a_{-s_1'} a_{q-\ell} a_q a_{r-\ell_1} (\NN+1)^{-\alpha}\xi_\lambda}^2\Bigg)^\half \times\nonumber\\
	& \quad \times \Bigg( \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^{2m} \Bigg)^{\half} k_{\F}^{-\frac 32} e(q)^{-1} \norm{ (\NN+5)^{\half + \alpha}\xi_\lambda } \nonumber\\
	&\leq C^m k_{\F}^{-\frac 32} e(q)^{-1}
		\norm{ (\NN+1)^{2-\alpha} a_q \xi_\lambda} \norm{ (\NN+5)^{\half+\alpha}\xi_\lambda } \nonumber \;.\\
\end{align}
Choosing $\alpha = 2$ and applying Lemma~\ref{lem:normsk} and $ (\cN+5)^m \le C (\cN+1)^m $, we get
\begin{equation}
	 \sum_{\ell,\ell_1 \in \Z^3_*} |\I_0(\ell, \ell_1)|
	 \leq C^m k_{\F}^{-\frac 32} e(q)^{-1} \Xi^\half
	 	\norm { (\NN+1)^{\frac 52} \xi_\lambda } \;. \label{eq:estEQ1111} 
\end{equation}
}
...
\begin{align}
	...
	&\quad \times \Bigg( \sum\limits_{r,\ell,\ell_1,s',s_1' \in \Z^3_*} \norm{a_{r-\ell_1} a_{s'} a_{s'-\ell} a_{-s_1'} a_{-s_1'+\ell_1} \xi_\lambda}^2 \Bigg)^\half
		\norm{a_{r-\ell} \xi_\lambda } \nonumber\\
	&\leq C^m k_{\F}^{-\frac 32} e(q)^{-1}
		\norm{(\NN+1)^{\frac 52} \xi_\lambda} \Xi^\half \;. \label{eq:estEQ1112}
\end{align}
}
...
\begin{aligned}
	...
	\leq C^m \big( k_{\F}^{-\frac 32 + \varepsilon}
...
\end{aligned}
\end{equation}
We again employ three different bounding strategies for $ j = 0 $, for $ 1 \le j \le m-1 $, and $ j = m $. For $ j = 0 $, we write again $1 = (\NN+1)^{-1}(\NN+1)$. Together with the Cauchy--Schwarz inequality, $ \norm{a_p} \le 1 $, and Lemma~\ref{lem:normsk}
\textcolor{green!30!black}{
\begin{align}
	&\sum_{\ell,\ell_1 \in \Z^3_*} |\I_0(\ell, \ell_1)| \nonumber\\
	&\leq \sum_{\ell,\ell_1 \in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ \cap (-L_{\ell_1}+\ell+\ell_1)}} \norm{ (\NN+5) \xi_\lambda} \norm{ K^m(\ell )_{r,q} K(\ell_1)_{r,-r+\ell+\ell_1} a_{r-\ell-\ell_1} a_{r-\ell_1} b_{q}(\ell) (\NN+1)^{-\alpha} \xi_\lambda }\nonumber\\
	 ...
	 &\quad \times \Bigg( \sum\limits_{\ell_1 \in \Z^3} \norm{ a_{r-\ell_1} a_q a_{q-\ell} (\NN+1)^{-1} \xi_\lambda }^2 \Bigg)^\half \nonumber\\
	 &\leq \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^m k_{\F}^{-1} e(q)^{-1}
	 	\norm{ (\NN+5) \xi_\lambda}
	 	\sum_{r \in \Z^3} \Bigg( \sum_{\ell_1 \in \Z^3_*} \frac{\mathds{1}_{L_{\ell_1} \cap L_{-\ell_1+\ell+\ell_1}}(r) \hat{V}(\ell_1)^2}{(\lambda_{\ell_1,r} + \lambda_{\ell_1,-r+\ell+\ell_1})^2} k_{\F}^{-2} \Bigg)^{\half} \times \nonumber\\
	 &\quad \times \norm{ a_q a_{q-\ell} (\NN+1)^{-\half} \xi_\lambda } \nonumber\\
	 &\leq \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^m k_{\F}^{-1} e(q)^{-1}
	 	\norm{ (\NN+5) \xi_\lambda}
	 	\sum_{r \in \Z^3} e(r)^{-1} k_{\F}^{-1} \Bigg( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \Bigg)^{\half} \times \nonumber\\
	 &\quad \times \norm{ a_q a_{q-\ell} (\NN+1)^{-\half} \xi_\lambda }  \;,
\end{align}
...
\begin{align}
	&\sum_{\ell,\ell_1 \in \Z^3_*} |\I_0(\ell, \ell_1)| \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^m k_{\F}^{-1+\varepsilon} e(q)^{-1}
	 	\norm{ (\NN+5) \xi_\lambda}
	 	\norm{ a_q a_{q-\ell} (\NN+1)^{-\half} \xi_\lambda } \nonumber\\
	&\leq \Bigg( \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^{2m} \Bigg)^{\half} k_{\F}^{-1+\varepsilon} e(q)^{-1}
	 	\norm{ (\NN+5) \xi_\lambda}
	 	\Bigg( \sum_{\ell \in \Z^3_*} \norm{ a_q a_{q-\ell} (\NN+1)^{-\half} \xi_\lambda }^2 \Bigg)^{\half} \nonumber\\
	 &\leq C^m k_{\F}^{-1+\varepsilon} e(q)^{-1}
	 	\norm{ (\NN+5) \xi_\lambda}
	 	\Xi^{\half}	\;.
	\label{eq:estEQ1121}
\end{align}
The bound for $ \sum_{\ell} \hat{V}(\ell) < \infty $ is considerably simpler and stronger:
\begin{align}
	&\sum_{\ell,\ell_1 \in \Z^3_*} |\I_0(\ell, \ell_1)| \nonumber\\
...
\begin{align}
	...
	&\leq \norm{ \xi_\lambda} 
		\Bigg( \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^{2m} \Bigg)^{\half} k_{\F}^{-\frac 32 + \varepsilon} e(q)^{-1}
		\Bigg( \sum\limits_{\ell,s \in \Z^3} \norm{a_s a_{s-\ell} (\NN+1)^{\half} \xi_\lambda }^2 \Bigg)^{\half} \nonumber\\
	&\leq C^m k_{\F}^{-\frac 32 + \varepsilon} e(q)^{-1} \norm{(\NN+5)^{\frac 32} \xi_\lambda }^2 \;.
\end{align}
}
...
\begin{align}
	...
	&\leq \norm{\xi_\lambda}
		...
		\Bigg( \sum\limits_{s \in \Z^3} \abs{K^m(\ell)_{q,s}}^2\Bigg)^\half \Bigg(\sum\limits_{s \in L_{\ell}} \norm{ a_{q-\ell_1} a_s a_{s-\ell}  \xi_\lambda }^2\Bigg)^\half \nonumber\\
	&\leq \norm{\xi_\lambda }
		\Bigg( \sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^{2m} \Bigg)^{\half}
		\Bigg( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \Bigg)^{\half}
		k_{\F}^{-\frac 32} e(q)^{-1} \times \nonumber\\
	&\quad \times \Bigg( \sum\limits_{\ell_1, \ell, s \in \Z^3} \abs{K^m(\ell)_{q,s}}^2\Bigg)^\half \Bigg(\sum\limits_{s \in L_{\ell}} \norm{ a_{q-\ell_1} a_s a_{s-\ell}  \xi_\lambda }^2\Bigg)^\half \nonumber\\
	&\leq C^m k_{\F}^{-\frac 32} e(q)^{-1} \norm{(\NN+1)^{\frac 32} \xi_\lambda } \;,
\end{align}
...
\end{align}
\textcolor{green!30!black}{Moreover, in the Coulomb case $ \hat{V}(\ell) \le C |\ell|^{-2} $, the following bound holds true for any $ \alpha \ge 0 $:
\begin{align}
	\abs{\eva{\xi_\lambda,\left(E^{m,1}_{Q_2}+E^{m,2}_{Q_2}+\mathrm{h.c.}\right) \xi_\lambda }}
	\leq C^m
		e(q)^{-1}
