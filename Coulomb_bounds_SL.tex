...
\begin{align}
	...
	&\quad \times \mathds{1}_{L_\ell}(q) \sum\limits_{s\in L_{\ell}} \Bigg( \sum_{\ell_1 \in \Z^3_*} \norm{K^{j}(\ell)_{q,s} a_{r-\ell_1} b_{s}(\ell) \xi_\lambda }^2 \Bigg)^\half \nonumber\\
	&\leq \norm{ \xi_\lambda} 
		\sum_{\ell \in \Z^3_*}
		(C \hat{V}(\ell))^{m-j} k_{\F}^{-1} e(q)^{-1}
...
\begin{align}
	...
	&\leq \norm{ (\NN+5)^{\half} \xi_\lambda}
		\sum_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) (C \hat{V}(\ell))^{m-j} k_{\F}^{-\frac 32} e(q)^{-1}
		\hat{V}(\ell_1)
...
\end{align}
For $ \sum_{\ell_1} \hat{V}(\ell_1) $, we may even achieve a bound of order $ \sim k_{\F}^{-2} $: With $ q \in L_\ell $,}
\begin{align}
	|\I_j(\ell, \ell_1)|
...
\begin{align}
	...
	&\leq \sum_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell \cap (L_{\ell_1}+\ell-\ell_1)}(q) \sum\limits_{r\in L_{\ell_1} \cap L_\ell}
		\norm{ K(\ell_1)_{r,q-\ell+\ell_1} a_{-q+\ell-\ell_1} a_{r-\ell_1} (\NN+1)^{\half} \xi_\lambda} \times \nonumber\\
...
\begin{align}
	&\sum_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) |\I_0(\ell, \ell_1)| \nonumber\\
	&\le \sum_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell \cap L_{\ell_1} \cap (-L_{\ell_1} + \ell + \ell_1) \cap (-L_\ell + \ell + \ell_1)}(q)
...
Let $ \sum_{\ell \in \Z_3^*} \hat{V}(\ell)^2 < \infty $. For $\xi_\lambda = e^{-\lambda S} \Omega$, given $ \varepsilon > 0 $ there exist $ C, C_\varepsilon > 0 $ such that for all $ \lambda \in [0,1] $, $ m \in \NNN $, $ m > 1 $ and $ q \in B_{\F}^c $,
\begin{equation}
	\abs{\eva{\xi_\lambda, n_q^{\ex,m} \xi_\lambda }}
	\leq C_\varepsilon C^m k_{\F}^{-\frac 32} e(q)^{-\frac 32} \;. \label{eq:estnqex_Coulomb}
\end{equation}
In case $ m = 1 $, if $ \sum_{\ell \in \Z_3^*} \hat{V}(\ell)^2 |k|^\alpha < \infty $ with $ \alpha \in [0,2] $, we have
\begin{equation}
	\abs{\eva{\xi_\lambda, n_q^{\ex,1} \xi_\lambda }}
	\leq C_\varepsilon k_{\F}^{-1 - \frac{\alpha}{2} + \varepsilon} e(q)^{-1} \;. \label{eq:estnqex_Coulomb_1}
\end{equation}
Further, if $ \sum_{\ell \in \Z_3^*} \hat{V}(\ell) < \infty $, we have the following stronger bound for any $ m \in \mathbb{N} $:
\begin{equation}
	\abs{\eva{\xi_\lambda, n_q^{\ex,m} \xi_\lambda }}
...
\end{aligned}
\end{equation}
We first consider $ m > 1 $, where $ \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^m < \infty $. Here, Lemma~\ref{lem:normsk} and $  \lambda_{\ell,q} \geq C e(q) $, $ \lambda_{\ell_1,q} \geq C e(q) $ give
\begin{equation}
\begin{aligned}
	\I
	...
	&\leq C k_{\F}^{-2} e(q)^{-\frac 32} \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^m
		\Bigg( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \Bigg)^{\half}
		\Bigg( \sum_{\ell_1 \in \Z^3_*} \mathds{1}_{-L_\ell + \ell + \ell_1}(q)
...
\begin{aligned}
	...
	&\leq C k_{\F}^{-3} e(q)^{-\frac 32} \sum_{\ell \in \Z^3_*} \sum_{r \in L_\ell}
		\hat{V}(\ell)^m
		\Bigg( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2  \Bigg)^{\half}
		\Bigg( \sum_{\ell_1 \in \Z^3_*} \mathds{1}_{-L_{\ell}+\ell+\ell_1}(r) \lambda_{\ell,-r+\ell+\ell_1}^{-1} \Bigg)^{\half}
		 e(r)^{-1} \\
...
Next, we consider $ m = 1 $, where $ \II = 0 $ as the sum over $ j $ is empty. Hence, with Lemma~\ref{lem:normsk},
\begin{equation}
\begin{aligned}
	|n_q^{\ex,1}|
	&\leq C k_{\F}^{-2}
...
\end{aligned}
\end{equation}
The first bracket was assumed to be finite. To bound the second bracket, we split the summation region as $ S := \{ \ell \in \Z^3_* : q \in L_\ell \} = S_1 \cup S_2 \cup S_3 $ with
\begin{equation}
...
\end{equation}
for some $ \beta > 0 $ to be optimized later. For $ \ell \in S_1 $, we use $ e(q-\ell) \ge C $ and that the area of the intersection with a shell around $ q $ is bounded by $ |S_1 \cap \partial B_{|\ell|}(q)| \le C |\ell| $:
\begin{equation}
	\sum_{\ell \in S_1} e(q-\ell)^{-1} |\ell|^{-\alpha}
	\leq \sum_{\ell \in S_1} |\ell|^{-\alpha}
	\leq C + C \int_1^{k_{\F}^\beta} |\ell|^{1-\alpha}  \di |\ell|
...
For $ \ell \in S_2 $, we have $ e(q-\ell) \ge C k_{\F} $, while $ |S_2 \cap \partial B_{|\ell|}(q)| \le C |\ell|^2 $:
\begin{equation}
	\sum_{\ell \in S_2} e(q-\ell)^{-1} |\ell|^{-\alpha}
	\leq C k_{\F}^{-1} \Bigg( 1 + \int_1^{k_{\F}^\beta} |\ell|^{2-\alpha} \di |\ell| \Bigg)
	\le C k_{\F}^{-1 + 3 \beta - \alpha \beta} \;.
\end{equation}
Finally, for $ \ell \in S_3 $,
\begin{equation}
	\sum_{\ell \in S_1} e(q-\ell)^{-1} |\ell|^{-\alpha}
...
Finally, in case $ \sum_{\ell \in \Z^3_*} \hat{V}(\ell) < \infty $, we get the simpler bound~\eqref{eq:estnqex} using again Lemma~\ref{lem:normsk} and $ \sum_{0 \le j \le m} {{m}\choose j} = 2^m $:
\begin{equation}
\begin{aligned}
	\I + \II
