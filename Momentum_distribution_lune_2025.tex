...
\begin{itemize}
\item $ \Psi_N $ is energetically close to the ground state in the sense that there exists a $ \varepsilon > 0 $ and a $ C_\varepsilon > 0 $ such that for all $ N $, we have
\begin{equation} \label{eq:main1}
	\eva{\Psi_N, H_N \Psi_N} - E_{\GS}
	\le C_\varepsilon k_{\F}^{1-\frac 16 + \varepsilon} \;,
\end{equation}
\item There exists a constant $ C_V > 0 $ such that for all $ N $ and $ q \in \Z^3 $, the momentum distribution in $ \Psi_N $ satisfies
...
\end{equation}
where we recall~\eqref{eq:EQ1EQ2extension} that $ E_{Q_1}\left(\Theta^m_{K}(P^q)\right) $ and $ E_{Q_2}\left(\Theta^m_{K}(P^q)\right) $ expand into 14 error terms and an exchange terms. We will consecutively bound these terms.
...
\end{equation}
\end{proposition}
\textcolor{red}{[SL: We must still subtract the contribution to $ n_q^{\ex} $ from $ E_m(P^q) $. Say, $ n_q^{\ex,m} $. Also, we should refer to the definitions of the $ E_{Q_1} $ and $ E_{Q_2} $-terms when we finalize the proofs of the two main propositions.]}
...
As in~\eqref{eq:EQ1111}, we split the anticommutator in $ E^{m,1}_{Q_2} $~\eqref{eq:expandedEQ2} using~\eqref{eq:q-q}:\todo{Check after re-defining $ E_{Q_2} $ that the factor of 4 is still correct.}
...
Finally, we need to bound $ n_q^{\ex,m} $, which also turns out to be sub-leading to $ n_q^{\b} \sim k_{\F}^{-1} e(q)^{-1} $.
...
\begin{proposition}[Control of the bosonized momentum distribution] \label{prop:nqb_bounds}
...
\begin{equation}
	...
	\le C k_{\F} \;.
\end{equation}
Plugging this bound into~\eqref{eq:main_errorbound_with_Xi} renders the desired result.
