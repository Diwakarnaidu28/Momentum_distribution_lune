...
\begin{equation} \label{eq:finalEmest}
	...
	\leq \frac{C^m}{m!} e(q)^{-1} \left( k_{\F}^{-\frac{3}{2}} \Xi^\half
...
\begin{equation} \label{eq:finalEQ1est}
	...
	\leq \Bigg(\sum\limits_{\ell \in \Z^3_*} (C \hat{V}(\ell))^m\Bigg)
...
\begin{equation}
	...
	\leq \Bigg( \sum\limits_{\ell \in \Z^3_*} (C \hat{V}(\ell))^{m+1} \Bigg)
...
\begin{align}
	...
	&\leq (C \hat{V}(\ell))^m \hat{V}(\ell_1) k_{\F}^{-2} e(q)^{-1} \Xi^{\half} \norm{ (\NN+1) \xi_\lambda} \;.
\end{align}
...
\begin{proof}
Splitting the anticommutator in $ E^{\,1,5}_{Q_2} $~\eqref{eq:???} via~\eqref{eq:q-q} gives
...
\begin{proof}
Splitting the anticommutator in $ E^{\,1,7}_{Q_2} $~\eqref{eq:???} via~\eqref{eq:q-q} gives
\begin{equation} \label{eq:EQ2171}
\begin{aligned}
	\abs{\eva{\xi_\lambda,\left(E^{\,1,7}_{Q_2}+\mathrm{h.c.}\right) \xi_\lambda }} 
	= 2\abs{\eva{\xi_\lambda, E^{\,1,7}_{Q_2} \xi_\lambda }}
	\le 4 \sum_{j=0}^m {{m}\choose j} \sum\limits_{\ell,\ell_1  \in \Z^3_*}\!\! \mathds{1}_{L_\ell}(q) |\I_j(\ell, \ell_1)| \;,\\
	\I_j(\ell, \ell_1)
...
\begin{proof}
We focus on $ E^{\,1,8}_{Q_2} $, since the proof for $ E^{\,2,8}_{Q_2} $ is analogous.
Splitting again the anticommutator in $ E^{\,1,8}_{Q_2} $~\eqref{eq:???} via~\eqref{eq:q-q} we get
\begin{equation} \label{eq:EQ2181}
\begin{aligned}
	\abs{\eva{\xi_\lambda,\left(E^{\,1,8}_{Q_2}+\mathrm{h.c.}\right) \xi_\lambda }} 
	= 2\abs{\eva{\xi_\lambda, E^{\,1,8}_{Q_2} \xi_\lambda }}
	\le 4 \sum_{j=0}^m {{m}\choose j} \sum\limits_{\ell,\ell_1  \in \Z^3_*}\!\! \mathds{1}_{L_\ell}(q) |\I_j(\ell, \ell_1)| \;,\\
	\I_j(\ell, \ell_1)
...
\end{align}
\textcolor{red}{[SL: Note that we could also sacrifice a $ \Xi^{\half} $, here, to gain another $ e(q)^{\half} $. This is not relevant here, but maybe an interesting observation.]}
\end{proof}
...
\begin{equation}
	...
	\le \int_{\Delta^m} \di^m\underline{\lambda} \;
		\abs{\eva{\xi_\lambda, E_{Q_{\sigma(m)}}\left(\Theta^{m}_{K}(P^q)\right) \xi_\lambda}} \;.
\end{equation}		
Using Propositions~\ref{prop:finEQ1est} and~\ref{prop:finEQ2est} with
\begin{equation}
	\sum_{\ell \in \Z^3_*} (C \hat{V}(\ell))^m
	\le \Bigg( C \sum_{\ell \in \Z^3_*} \hat{V}(\ell) \Bigg)^m
	\le C^m \Vert \hat{V} \Vert_1^m
	\le C^m \;,
\end{equation}
we arrive at
\begin{equation}
	\abs{\eva{\xi_\lambda, E_{Q_{\sigma(m)}}\left(\Theta^{m}_{K}(P^q)\right) \xi_\lambda}}
	\le C^m e(q)^{-1} \left( k_{\F}^{-\frac{3}{2}} \Xi^\half
		+ k_{\F}^{-1}\Xi^{\frac{3}{4}} \right) \;,
\end{equation}
