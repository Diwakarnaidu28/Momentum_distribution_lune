...
\end{equation}
with $ K^0 = I $\todo{Unify the notation for the identity $ \mathrm{id} $ vs. $ I $ vs. $ \mathds{1} $}, we obtain
...
In this section, we prove the claimed scalings (???) of the leading-order terms $ n_q^{\b} $~\eqref{eq:???} and $ n_q^{\ex} $~\eqref{eq:???}.
...
\begin{equation}
	...
	\le C \sum_{\ell \in \Z^3_*} \frac{k_{\F}^{-1}}{\lambda_{\ell,q}} \sum_{m=1}^{\infty} \frac{4^m \hat{V}(\ell)^{2m}}{(2m)!}
...
For the lower bound, first observe that~\cite[Prop.~7.8]{CHN21} implies the matrix element bound
\begin{equation}
	(\cosh(2K(\ell)) - 1)_{q,q}
	\ge \frac{c \hat{V}(\ell) k_{\F}^{-1}}{\lambda_{\ell,q}}
...
\end{equation}
As a potential, we now choose $ \hat{V}((1,0,0)) = 1 $ and $ \hat{V}(\ell) = 0 $ on all other momenta $ \ell \neq (1,0,0) $. Then, we choose $ k_{\F}^{(n)} $ slightly larger than $ n $, such that $ (0,0,n) \in B_{\F} $ while $ q_n := (1,0,n) \notin B_{\F} $ with $ e(q_n) \ge \half $. Evidently, with $ \ell^* := (1,0,0) $ we then get $ \lambda_{\ell^*, q_n} = 1 $, so
\begin{equation}
	n_q^{\b}
