...
\begin{align} \label{eq:finexpan}
	...
	&= n_q^{\b}
		...
		+\half \int_{\Delta^{n+1}} \di^{n+1}\underline{\lambda} \;
...
We split the proof into two parts: First, we execute the Duhamel expansion, recovering some expression for $ n_q^{\b} $ in terms of the matrix $ K $~\eqref{eq:K}. Then, we show that this expression coincides with the integral in~\eqref{eq:nqb}

\begin{lemma}[Duhamel expansion of the excitation density] \label{lem:Duhamelexpan}
For $q \in B^c_{\F}$, we have
\begin{align} \label{eq:Duhamelexpan}
	\eva{\Omega, e^{\cK} a_q^* a_q e^{-\cK} \Omega} 
...
\end{aligned}
\end{equation}
Using reflection symmetry, we may replace $ \tilde{P}^q $ by $ P^q $. The desired result then follows, noticing that by cyclicity of the trace,
\begin{equation}
	\sum\limits_{m=1}^{\floor{(n+1)/2}} \frac{\mathrm{Tr} \,\Theta^{(2m)}_K (P^q)}{(2m)!}
	= \sum\limits_{m=1}^{\floor{(n+1)/2}} \frac{2^{2m} K^{2m}(\ell)_{q,q}}{(2m)!}
	= \big( \cosh(2K(\ell)) - 1 \big)_{q,q} \;.
\end{equation}
\end{proof}
...
\begin{equation}
	\half (\cosh(2K(\ell))-1)_{q,q} = \frac{1}{\pi} \int_0^\infty  \frac{2g_\ell (t^2-\lambda_{\ell,q}^2) (t^2+\lambda^2_{\ell,q})^{-2}}{1+ 2g_\ell\sum_{p \in L_{\ell}}\lambda_{\ell,p}(t^2+\lambda^2_{\ell,p})^{-1} }  \mathrm{d}t \;.
\end{equation}
Summing over $ \ell \in \Z^3_* $ with $ q \in L_\ell $ and comparing with~\eqref{eq:nqb}, we get the claimed result.
\end{proof}

\begin{proof}[Proof of Proposition~\ref{prop:finexpan}]
Follows readily by~\eqref{eq:Duhamelexpan} and~\eqref{eq:nqb_integralrecovery}.
\end{proof}
...
\end{aligned}
\end{equation}
We will need to employ three different estimation strategies for $ j = 0 $, $ 1 \le j \le m-1 $ and $ j = m $. For $ j = 0 $, we start with splitting $1 = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for some $\alpha \in \R$ to be fixed later. Then we use the Cauchy-Schwarz inequality and Lemma~\ref{lem:pairest}.
\begin{align}
	&|\I_0(\ell, \ell_1)| \nonumber\\
...
For $ q \in \Z^3 $, $\xi_\lambda = e^{-\lambda \cK} \Omega$ and $ \lambda \in [0,1] $, there exists a constant $ C > 0 $ such that for all $ m \in \NNN $,
\begin{equation}\label{eq:genEQ2est}
	\abs{\eva{\xi_\lambda, E_{Q_2}\!\left(\Theta^m_K(P^q)\right)  \xi_\lambda}} 
	\leq C \left( k_{\F}^{-\frac{3}{2}} \Xi^\half
		+ k_{\F}^{-1}\Xi^{\frac{3}{4}}
		+ k_{\F}^{-1}\Xi \right)
		\Bigg(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^m\Bigg)\Bigg( \sum\limits_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1) \Bigg) \;. \\
\end{equation}
...
\begin{proof}
  ...
  For \eqref{eq:EQ2111}-\eqref{eq:EQ2113}, we start by using resolution of the identity $1 = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for some $\alpha \in \R$. Then we use the Cauchy-Schwarz inequality, Lemma \ref{lem:commNab} and the bounds from Lemma \ref{lem:pairest}.We estimate \eqref{eq:EQ2111} as
\begin{align}
	&\eqref{eq:EQ2111}\nonumber\\
...
\begin{proof}
    ...
    \end{align}
    where the last inequality is implied by Remark \ref{q-q} and we used \eqref{eq:decomptheta}.
    For \eqref{eq:EQ2121}-\eqref{eq:EQ2123}, we start by using resolution of the identity $1 = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for some $\alpha \in \R$. Then we use the Cauchy-Schwarz inequality, Lemma \ref{lem:commNab} and the bounds from Lemma \ref{lem:pairest}.
...
\end{align}
where the last inequality is implied by Remark \ref{q-q} and we used \eqref{eq:decomptheta}.
For \eqref{eq:EQ2131}-\eqref{eq:EQ2133}, we start by using resolution of the identity $1 = (\NN+1)^{-\alpha}(\NN+1)^{\alpha}$ for some $\alpha \in \R$. Then we use the Cauchy-Schwarz inequality and the bounds from Lemma \ref{lem:pairest}.
...
\subsection{Control of the Leading-Order Terms}
...
\end{proposition}
\begin{proof}
We focus on the case $ q \in B_{\F}^c $, as $ q \in B_{\F} $ is treated analogously. For the lowed bound on $ n_q^{\b} $, we expand the $ \cosh $ in~\eqref{eq:???} and use Lemma~\ref{lem:normsk}, as well as $ 2 \lambda_{\ell,q} = e(q) + e(q - \ell) \ge e(q) $
\begin{equation}
	n_q^{\b}
