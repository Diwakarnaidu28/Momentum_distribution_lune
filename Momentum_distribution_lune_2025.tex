...
\end{equation}
\item and there exists a constant $ C > 0 $, depending on $ \Vert \hat{V} \Vert_1 $, such that for all $ k_{\F} $ and $ q \in \Z^3 $, the momentum distribution in $ \Psi_N $ satisfies
...
\item In analogy to~\cite[Sect.~1.1]{BL25}, in $ n_q^{\b} $~\eqref{eq:nqb}, we may approximate $ \lambda{\ell,q} \approx 2 k_{\F} |\hat{\ell} \cdot \hat{q}| $ with $ \hat{q} := \frac{q}{|q|} $, then set $ \mu := t (2 k_{\F} |\ell|) $ and take the continuum limits $ \sum_{p \in L_\ell} \approx \int_{L_\ell} \di p $ and $ \sum_\ell \approx \int \di \ell $. The result is 
\begin{equation}
\begin{aligned}
	n_q^{\b}
	&\approx \int_{\R^3} \di \ell \; \mathds{1}_{L_{\ell}}(q) \; \frac{\hat{V}(\ell) k_{\F}^{-2}}{\pi (2 \pi)^3 |\ell|}
...
\end{aligned}
\end{equation}
\textcolor{red}{This does not match with~\cite{BL25}!!!}
