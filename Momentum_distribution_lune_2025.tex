...
\begin{align}
	...
	\leq C k_{\F}^{-\frac{3}{2}} e(q)^{-\frac 32} \Xi^{\half} \left(\sum\limits_{\ell\in \Z^3_*} \hat{V}(\ell)^m \right) \left(\sum\limits_{\ell_1\in \Z^3_*} \hat{V}(\ell_1) \right) \norm{(\NN+1)^\half \xi_\lambda } \;. \label{eq:estEQ217}
\end{align}
\end{lemma}
...
\begin{align}
	...
	\leq C k_{\F}^{-2} e(q)^{-\frac 32} \Xi \left(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^m \right) \left(\sum\limits_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1) \right) \;.\label{eq:estEQ218}  
\end{align}
\end{lemma}
...
\begin{equation}
	...
	\leq C k_{\F}^{-2} e(q)^{-2} \Xi
		\hat{V}(\ell)^m
		\hat{V}(\ell_1) \;. \label{eq:estEQ2181}
\end{equation}
...
\begin{align}
	...
	&\leq C k_{\F}^{-2} e(q)^{-\frac 32} \Xi
		\hat{V}(\ell)^m
		\hat{V}(\ell_1) \;. \label{eq:estEQ2182}
\end{align}
...
\begin{proof}[Proof of Proposition~\ref{prop:finEQ2est}]
We add the bounds from Lemmas~\ref{lem:EQ211}--\ref{lem:EQ218}, and use $ e(q) \ge 1 $, $ \Xi \le 1 $, as well as the Gr\"onwall bound, Lemma \ref{lem:gronNest}, to estimate $ \norm{(\NN+1)^2 \xi_\lambda} \le C $.
\end{proof}
