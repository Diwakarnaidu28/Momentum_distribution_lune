...
\begin{equation} \label{eq:K}
	K(k) \coloneq - \half \log \Big( h(k)^{-\half}
		\big( h(k)^{\half} \big( h(k) + 2 P(k) \big)^{\half} h(k)^{\half}\big)
		h(k)^{-\half} \Big) \;,
\end{equation}
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
The first bounds in~\eqref{eq:K_max_bounds} and~\eqref{eq:e(q)_extraction_bounds} then follow immediately, noting that $ 2 \lambda_{\ell,q} \ge e(q) \ge \half $. The second bounds in~\eqref{eq:K_max_bounds} and~\eqref{eq:e(q)_extraction_bounds} follow from
\begin{equation}
\begin{aligned}
...
\begin{proof}
We focus on the case $ q \in B_{\F}^c $, as $ q \in B_{\F} $ is treated analogously. For the lowed bound on $ n_q^{\b} $, we use the sum representation~\eqref{eq:nqb_integralrecovery}, where we expand the $ \cosh $ and use Lemma~\ref{lem:normsk}, as well as $ 2 \lambda_{\ell,q} = e(q) + e(q - \ell) \ge e(q) $
\begin{equation}
	n_q^{\b}
...
\begin{equation}
	...
	\le C k_{\F} \;.
\end{equation}
Plugging this bound into~\eqref{eq:main_errorbound_with_Xi} renders the desired formula~\eqref{eq:main2}. The claim $ |n_q^{\b}| \le C k_{\F} e(q)^{-1} $ was already proven in Lemma~\ref{lem:nqb_bounds}
