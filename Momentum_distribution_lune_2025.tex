...
\subsection{Extraction of the Exchange Contribution}
...
The exchange contribution $ n_q^{\ex} $ in (???) now follows as
\begin{equation}
	n_q^{\ex}
...
The main bound of this subsection is then the following: \textcolor{red}{[SL: We must still subtract the contribution to $ n_q^{\ex} $ from $ E_m(P^q) $. Say, $ n_q^{\ex,m} $.]}

\begin{proposition} \label{prop:finalEmest}
Recall the bosonization error term $E_m(P^q)$\todo{subtract the $ n_q^{\ex} $}~\eqref{eq:errEm} with $ \Theta^n_K $, $ P^q $, $ \int_{\Delta^n} \di^n \ulambda $ and $ \sigma(n) $ defined within and above Proposition~\ref{prop:finexpan}. Then, there exists a constant $ C > 0 $ such that for all $ m \in \NNN $ and $ q \in B_{\F}^c $,
...
We add the bounds from Lemmas~\ref{lem:EQ211}--\ref{lem:EQ218}, and use $ e(q) \ge \half $, $ \Xi \le 1 $, as well as the Gr\"onwall bound, Lemma \ref{lem:gronNest}, to estimate $ \norm{(\NN+1)^2 \xi_\lambda} \le C $.
\end{proof}
