...
\begin{proof}
The proof strategy is very similar to the one of Lemma~\ref{lem:EQ111}: As in~\eqref{eq:EQ1111}, we split the anticommutator in $ E^{\,1,1}_{Q_2} $~\eqref{eq:???} using~\eqref{eq:q-q}:
...
\end{aligned}
\end{equation}
The estimation for $ j = 0 $ is again done with $1 = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for $\alpha \in \R$, the Cauchy--Schwarz inequality and Lemma~\ref{lem:pairest}:
\begin{align}
	|\I_0(\ell, \ell_1)|
