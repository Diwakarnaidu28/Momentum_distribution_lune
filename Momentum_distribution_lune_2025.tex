...
\end{equation}
\item There exists a constant $ C > 0 $, depending on $ \Vert \hat{V} \Vert_1 $, such that for all $ N $ and $ q \in \Z^3 $, the momentum distribution in $ \Psi_N $ satisfies
...
\begin{equation} \label{eq:nqex}
	...
	\coloneq \sum_{\substack{m=1\\m:\textnormal{ odd}}}^\infty \frac{1}{(m+1)!} \sum_{j=0}^m {{m} \choose {j}}
		\sum\limits_{\substack{\ell,\ell_1 \in \Z^3_* \\ q \in L_\ell}}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ \cap (-L_{\ell}+\ell+\ell_1) \\ \cap (-L_{\ell_1}+\ell+\ell_1 )}} \!\!\!
		K^{m-j}(\ell)_{r,q}
...
\textcolor{red}{[SL: $ S $ corresponds to $ - \cK $ in sn-article.]}
...
\end{equation}
with $ b_p(k) $ defined in~\eqref{eq:b}, and with commutation error \textcolor{red}{[SL: Some signs changed in this and the subsequent lemmas, when replacing $ e^{S} \mapsto e^{-S} $.}
\begin{equation}
	\epsilon_{p,q}(k,\ell)
...
\begin{equation} \label{eq:Theta}
	...
	\coloneq A \;,
\end{equation}
where $ A, K $ are understood either as matrices $ A: \ell^2(L_\ell) \to \ell^2(L_\ell) $, or families thereof, i.e., $ A = (A(\ell))_{\ell \in \Z^3_*} $. We further introduce the projection matrix \textcolor{red}{[SL: I changed this notation: $ P_q $ is now only the projection to $ q $, while $ \tilde{P}_q $ is the projection to $ q $ and $ -q $. This makes the proposition easier to read and avoids a later discussion, why we only need to fix $ q $ when writing down the error terms.]}
...
\begin{aligned}
	...
	\norm{A(\ell)}_{\max,2}
	\coloneq \sup\limits_{q \in L_\ell}\bigg(\sum\limits_{p \in L_\ell}\abs{A(\ell)_{p,q}}^2\bigg)^\half \;, \\
	\norm{A(\ell)}_{\mathrm{max,1}}
	&\coloneq \sup\limits_{q \in L_\ell}\sum\limits_{p \in L_\ell}\abs{A(\ell)_{p,q}} \;.
\end{aligned}
\end{equation}
...
\begin{equation}\label{eq:estopb}
	\sum\limits_{p \in L_\ell}\norm{b_p(\ell)\Psi}^2 \leq  \eva{\Psi, \NN\Psi} \;.
\end{equation}
...
\begin{lemma}[Bound on nested anticommutator]\label{lem:multicommest}
...
\begin{aligned}
	&\abs{\int_{\Delta^n} \di^n \ulambda \;
		\eva{\Omega, e^{\lambda_n S} Q_{\sigma(n)}(\Theta^n_{K}(P^q)) e^{-\lambda_n S} \Omega} } \\
	&\leq 2^n \int_{\Delta^n} \di^n \ulambda \,  \norm{K(\ell)}^n_{\mathrm{op}} \norm{P^q(\ell)}_{\HS} 
...
For $\xi_\lambda = e^{-\lambda S} \Omega$, there exists a constant $ C > 0 $ such that for all $ m \in \NNN $, $ \lambda \in [0,1] $, and $ q \in B_{\F}^c $,
\begin{equation} \label{eq:finalEQ1est}
	...
	\leq C^m \Bigg(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^m\Bigg)
		...
		e(q)^{-1} \left( k_{\F}^{-\frac{3}{2}} \Xi^\half + k_{\F}^{-1}\Xi^{\frac{3}{4}} \right) \;.
\end{equation}
\end{proposition}

To prove this proposition, we need to control the error terms $ E^{m,1}_{Q_1} $, $ E^{m,2}_{Q_1} $, and $ E^{m,3}_{Q_1} $.
...
\begin{align}
    ...
    &\leq \Bigg( \sum\limits_{r \in L_{\ell_1}} \Bigg\Vert \sum\limits_{s_1 \in L_{\ell_1}} K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) b_{q}(\ell) a_{r-\ell_1} (\NN+1)^{-\alpha}\xi_\lambda \Bigg\Vert^2\Bigg)^\half \times\nonumber\\
    &\quad \times \Bigg( \sum\limits_{r \in L_\ell}  \norm{  K^{m}(\ell)_{r,q}  a_{r-\ell} (\NN+5)^{\alpha}\xi_\lambda }^2\Bigg)^\half \nonumber\\
    &\leq \Bigg( \sum\limits_{r,s_1 \in L_{\ell_1}}\abs{K(\ell_1)_{r,s_1}}^2 \sum\limits_{s_1 \in L_{\ell_1}} \norm{ b_{-s_1}(-\ell_1) b_{q}(\ell) a_{r-\ell_1} (\NN+1)^{-\alpha}\xi_\lambda}^2\Bigg)^\half \times\nonumber\\
...
In case $ 1 \le j \le m-1 $, we again employ the Cauchy--Schwarz inequality and Lemmas~\ref{lem:normsk} and~\ref{lem:pairest}, but convert an operator $ a_r $ instead of $ a_q $ into the bootstrap quantity $ \Xi $:
\begin{align}
	&|\I_j(\ell, \ell_1)| \nonumber\\
	&\leq \sum\limits_{r\in L_{\ell} \cap L_{\ell_1}}\! \Bigg( \sum\limits_{s \in L_\ell}\abs{K^{j}(\ell)_{q,s}}^2\Bigg)^\half
		\Bigg\Vert \sum\limits_{s_1 \in L_{\ell_1}}  K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1)  a_{r-\ell_1}(\NN+1)^\half\xi_\lambda \Bigg\Vert
	\norm{  K^{m-j}(\ell)_{r,q} a_{r-\ell} \xi_\lambda } \nonumber\\
	&\leq (C \hat{V}(\ell))^j k_{\F}^{-\half} e(q)^{-\half} \Bigg( \sum\limits_{r \in L_{\ell_1}}
		\Bigg\Vert \sum\limits_{ s_1 \in L_{\ell_1}}  K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1)  a_{r-\ell_1}(\NN+1)^\half\xi_\lambda \Bigg\Vert^2\Bigg)^\half \times\nonumber\\
...
\end{align}
For $ j = m $, we employ a similar estimation strategy, using that $ [a_q^* a_q, \cN] = 0 $ implies
\begin{equation}
	\norm{a_q (\cN+1) \xi_\lambda}^2
	\le \eva{\xi_\lambda, a_q^* a_q a_q^* a_q \xi_\lambda}^{\half}
		\eva{\xi_\lambda, (\cN+1)^4 \xi_\lambda}^{\half}
	= \norm{a_q \xi_\lambda} \norm{(\cN+1)^2 \xi_\lambda} \;,
\end{equation}
whence
\begin{align}
	|\I_m(\ell, \ell_1)|
...
\begin{align}
	...
	&\leq (C \hat{V}(\ell))^{m-j} k_{\F}^{-1} e(q)^{-1}
		\Bigg( \sum\limits_{r,s_1\in L_{\ell_1}} 
		|K(\ell_1)_{r,s_1}|^2
		\sum\limits_{s_1'\in L_{\ell_1}}
		\norm{b_{-s_1'}(-\ell_1) \xi_\lambda}^2 \Bigg)^{\half} \times \nonumber\\
		&\quad \times \Bigg( \sum\limits_{r\in (-L_{\ell}+\ell+\ell_1)} 
...
\begin{align}
	...
	&\leq (C \hat{V}(\ell))^m \hat{V}(\ell_1) k_{\F}^{-\frac 52} e(q)^{-2}
...
\end{aligned}
\end{equation}
For $ j = 0 $, the term $ K^0(\ell)_{q,-r+\ell+\ell_1} = \delta_{q,-r+\ell+\ell_1} $ eliminates the sum over $ r $, so for $ q \in (-L_{\ell_1} + \ell + \ell_1) \cap (-L_\ell + \ell + \ell_1) $ with the Cauchy--Schwarz inequality and Lemma~\ref{lem:normsk} we immediately get
\begin{equation}
	|\I_0(\ell, \ell_1)|
	= \abs{\eva{ K(\ell_1)_{-q+\ell+\ell_1,q} a_{-q+\ell_1} \xi_\lambda, K^m(\ell)_{-q+\ell+\ell_1,q} a_{-q+\ell_1} \xi_\lambda }}
	\leq (C \hat{V}(\ell))^m
		\hat{V}(\ell_1)
