...
\end{align}
where $ a_q^*, a_q $ are the standard fermionic creation and annihilation operators of momentum $ q \in \ZZZ^3 $. As the momentum distribution of the true ground state is difficult to access~\cite{BL25}, we focus our attention on a trial state $ \Psi = \Psi_{\trial} $ that was constructed in~\cite{CHN23}.\\
...
\end{equation}
The errors will depend on inverses of the excitation energy
\begin{equation} \label{eq:eq}
	e(q)
	:= \abs{|q|^2 - \inf_{p \in B_{\F}^c} |p|^2 + \half}
	= \abs{|q|^2 - \sup_{h \in B_{\F}} |h|^2 - \half} \;,
\end{equation}
which is evidently lower bounded by $ e(q) \ge \half $.

\begin{theorem}[Main result] \label{thm:main}
Assume that the Fourier transformation of the interaction potential is $ \hat{V} \in \ell^1(\Z^3) $, $ \hat{V} \ge 0 $, $ \hat{V}(0) = 0 $ \textcolor{red}{and $ \hat{V}(k) = \hat{V}(-k) $}.
...
\begin{equation} \label{eq:main2}
	\begin{cases}
	\abs{\eva{\Psi_N, a_q^* a_q \Psi_N} - n_q^{\b}}
	&\le C k_{\F}^{-1 -\frac 34} e(q)^{-1} \quad
		\textnormal{for } |q| \ge k_{\F} \\
	\abs{1 - \eva{\Psi_N, a_q^* a_q \Psi_N} - n_q^{\b}}
	&\le C k_{\F}^{-1 -\frac 34} e(q)^{-1} \quad
		\textnormal{for } |q| \ge k_{\F} 
	\end{cases} \;,
\end{equation}
where $ n_q^{\b} $, defined in~\eqref{eq:nqb} satisfies $ |n_q^{\b}| \le C k_{\F}^{-1} e(q)^{-1} $.
\end{itemize} 
\end{theorem}
...
Let us briefly recap the trial state construction of~\cite{CHN23}. To facilitate notation, we work in second quantization by introducing the fermionic Fock space
\begin{equation}
...
\end{aligned}
\end{equation}
leading to Proposition~\ref{prop:finexpan}, which is the main result of this section. Note that $ a_q \Omega = 0 $, from which we immediately conclude $ \langle \Omega, a_q^* a_q \Omega \rangle = 0 $. The multicommutators are computed using the CAR~\eqref{eq:CAR}, where we extract $ n_q^{\b} $ by bosonization, similarly as in~\cite{BL25}. The term $ n_q^{\ex} $ then appears when normal ordering the bosonization errors, similar to the exchange contribution to the correlation energy appearing in~\cite{CHN23}. Since $ n_q^{\ex} \ll n_q^{\b} $ (see Lemma~\ref{lem:estnqex} and Lemma~\ref{lem:nqb_bounds}), we treat also $ n_q^{\ex} $ as a bosonization error.
...
\begin{equation} \label{eq:EQ1EQ2extension}
	...
	= \sum_{j=1}^{11} E_{Q_1}^{m,j} + \mathrm{h.c.} + n_q^{\ex,m} \;,
\end{equation}
with
\begin{align}
	E_{Q_1}^{m,1}
	&:= -2 \sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell},s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)(\ell)_{r,s} K(\ell_1)_{r,s_1} a^*_{r-\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_{r-\ell} 
	\;, \nonumber\\
	E_{Q_1}^{m,2}
	...
	\;, \nonumber\\
	E_{Q_1}^{m,3}
	&:= \textcolor{red}{+} 2 \sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1} \cap (-L_{\ell_1}+\ell+\ell_1)\\ s \in L_{\ell}}} \Theta^m_{K}(P^q)(\ell)_{r,s}K(\ell_1)_{r,-r+\ell+\ell_1} b^*_{s}(\ell) a^*_{r-\ell_1}a^*_{r-\ell-\ell_1} \;. \label{eq:expandedEQ1}
\end{align}
and
\begin{align}
	E_{Q_2}^{m,1}
	&:= 2\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell},s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)_{r,s}K(\ell_1)_{r,s_1} a^*_{r-\ell_1}b^*_{-s_1}(-\ell_1)b_{-s}(-\ell)a_{r-\ell} \;, \nonumber\\
	E_{Q_2}^{m,2}
	&:= 2\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in (L_{\ell}-\ell) \cap (L_{\ell_1}-\ell_1)\\ s \in L_{\ell},s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)_{r+\ell,s} K(\ell_1)_{r+\ell_1,s_1} a^*_{r+\ell_1} b^*_{-s_1}(-\ell_1) b_{-s}(-\ell) a_{r+\ell}\;, \nonumber\\
	E_{Q_2}^{m,3}
	&:= -2\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1} \cap (-L_{\ell_1}+\ell+\ell_1)\\ s \in L_{\ell}}} \Theta^m_{K}(P^q)_{r,s} K(\ell_1)_{r,-r+\ell+\ell_1}  a^*_{r-\ell_1}a^*_{r-\ell-\ell_1}b_{-s}(-\ell)\;, \nonumber\\
	E_{Q_2}^{m,4}
	&:= -2 \sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\cap (-L_{\ell}+\ell+\ell_1)\\s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)_{r,-r+\ell+\ell_1} K(\ell_1)_{r,s_1} b^*_{-s_1}(-\ell_1)a_{r-\ell-\ell_1}a_{r-\ell}\;, \nonumber\\
	E_{Q_2}^{m,5}
	&:= - 2\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in (L_{\ell}-\ell) \cap (L_{\ell_1}-\ell_1)}} \Theta^m_{K}(P^q)_{r,s+\ell}K(\ell_1)_{r,s+\ell_1}a^*_{r-\ell_1}a^*_{-s-\ell_1} a_{-s-\ell}a_{r-\ell}\;, \nonumber\\
	E_{Q_2}^{m,6}
	&:= -\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{r,s\in L_{\ell} \cap L_{\ell_1}} \Theta^m_{K}(P^q)_{r,s}K(\ell_1)_{r,s}a^*_{r-\ell_1}a^*_{-s+\ell_1} a_{-s+\ell}a_{r-\ell}\;, \nonumber\\
	E_{Q_2}^{m,7}
	&:= -\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{r,s\in (L_{\ell}-\ell)\cap (L_{\ell_1}-\ell_1)} \Theta^m_{K}(P^q)_{r+\ell,s+\ell} K(\ell_1)_{r+\ell_1,s+\ell_1}  a^*_{r+\ell_1}a^*_{-s-\ell_1}a_{-s-\ell}a_{r+\ell}\;, \nonumber\\
	E_{Q_2}^{m,8}
	&:= -2\sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\\cap (-L_{\ell}+\ell+\ell_1) \cap (-L_{\ell_1}+\ell+\ell_1)}} \Theta^m_{K}(P^q)_{r,-r+\ell+\ell_1}K(\ell_1)_{r,-r+\ell+\ell_1} a^*_{r-\ell_1}a_{r-\ell_1}\;, \nonumber\\
	E_{Q_2}^{m,9}
	&:= -2\sum\limits_{\ell,\ell_1 \in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\\cap (-L_{\ell}+\ell +\ell_1) \cap (-L_{\ell_1}+\ell+\ell_1)}} \Theta^m_{K}(P^q)_{r,-r+\ell+\ell_1}K(\ell_1)_{r,-r+\ell+\ell_1} a^*_{r-\ell-\ell_1}a_{r-\ell-\ell_1} \;, \nonumber\\
	E_{Q_2}^{m,10}
	&:= \textcolor{red}{1} \sum\limits_{\ell \in \Z^3_*} \sum\limits_{r\in L_{\ell}}\Theta^{m+1}_{K}(P^q)(\ell)_{r,r} a^*_{r-\ell}a_{r-\ell} \;, \nonumber\\
	E_{Q_2}^{m,11}
	&:= \textcolor{red}{1} \sum\limits_{\ell \in \Z^3_*} \sum\limits_{r\in L_{\ell}}\Theta^{m+1}_{K}(P^q)(\ell)_{r,r} a^*_{r}a_{r} \;, \nonumber\\
	n_q^{\ex,m}
	&:= \sum\limits_{\ell,\ell_1 \in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ \cap (-L_{\ell}+\ell+\ell_1) \\ \cap (-L_{\ell_1}+\ell+\ell_1 )}} \!\!\!\Theta^m_{K}(P^q)(\ell)_{r,-r+\ell+\ell_1}K(\ell_1)_{r,-r+\ell+\ell_1} + \mathrm{h.c.} \;, \label{eq:expandedEQ2}
\end{align}
\end{lemma}
\begin{proof}
The result follows by a lengthy but straightforward calculation.
\end{proof}
...
In particular, the exchange contribution $ n_q^{\ex} $ in (???) follows as
\begin{equation}
	n_q^{\ex}
	= \sum_{m=1}^\infty \frac{1}{m!} n_q^{\ex,m} \;.
\end{equation}
...
We now compile some estimates which we need to bound the many-body error terms in~\eqref{eq:finexpan}. We start with some bounds on norms of powers of the correlation structure $ K(\ell) $.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
The first bounds in~\eqref{eq:K_max_bounds} and~\ref{eq:e(q)_extraction_bounds} then follow immediately noting that $ \lambda_{\ell,r} \ge 1 $ and $ \lambda_{\ell,q} \ge e(q) \ge \half $. The second bounds in ~\eqref{eq:K_max_bounds} and~\ref{eq:e(q)_extraction_bounds} follow from
\begin{equation}
\begin{aligned}
	\sum_{r \in L_\ell} |(K(\ell)^m)_{q,r}|^2
	&\le \sum_{r \in L_\ell} (C \hat{V}(\ell))^{2m} k_{\F}^{-2} (\lambda_{\ell,r} + \lambda_{\ell,q})^{-2}
	\le (C \hat{V}(\ell))^{2m} k_{\F}^{-2} \lambda_{\ell,q} \sum_{r \in L_\ell} \lambda_{\ell,r}^{-1} \\
...
\end{equation}
For $ \Psi_0 \in \cF $ and $ \Psi_\lambda := e^{-\lambda \cK} \Psi_0 $, using the definition~\eqref{eq:T} of $ \cK $, then the Cauchy--Schwarz inequality and then Lemma~\ref{lem:pairest}, we get
\begin{align}
...
To prove this bound, we introduce the parametrized excitation vector $ \xi_\lambda := e^{- \lambda \cK} \Omega $ to write
...
\begin{aligned}
	...
	\le 4 \sum_{j=0}^m {{m}\choose j} \sum\limits_{\ell,\ell_1  \in \Z^3_*}\!\! \mathds{1}_{L_\ell}(q) |\I_j(\ell, \ell_1)| \;,\\
	\I_j(\ell, \ell_1)
	:= \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ \cap (-L_{\ell_1}+\ell+\ell_1)\\ s_1 \in L_{\ell_1}}} 
...
\begin{aligned}
	...
	\le 4 \sum_{j=0}^m {{m}\choose j} \sum\limits_{\ell,\ell_1  \in \Z^3_*}\!\! \mathds{1}_{L_\ell}(q) |\I_j(\ell, \ell_1)| \;,\\
	\I_j(\ell, \ell_1)
	:= \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\\cap (-L_{\ell}+\ell+\ell_1) \\ \cap (-L_{\ell_1}+\ell+\ell_1)}}
...
Recall the definitions of the bosonized excitation density $ n_q^{\b} $~\eqref{eq:nqb}, as well as of the excitation energy $ e(q) := ||q|^2 - k_{\F}^2| $. Then, for any fixed potential $ \hat{V} \in L^1(\Z^3_*) $, there exists a constant $ C > 0 $ such that for all particle numbers $ N = |B_{\F}| \sim k_{\F}^3 $ and all $ q \in \Z^3 $,
