...
\begin{equation}
	...
	\le \sup_{q \in \Z^3} \sum_{r \in L_\ell} (C \hat{V}(\ell))^{2m} k_{\F}^{-2} (\lambda_{\ell,r} + \lambda_{\ell,r+s})^{-2}
	\le (C \hat{V}(\ell))^{2m} k_{\F}^{-2} \sum_{r \in L_\ell} \lambda_{\ell,r}^{-1}
	\le (C \hat{V}(\ell))^{2m} k_{\F} \;.
\end{equation}
\end{proof}
...
\begin{align}
	...
	&\quad \times \left( \sum\limits_{\ell_1, r \in \Z^3_*}
		...
		\norm{(\NN+5)^\alpha \psi} \;. \nonumber\\
\end{align}
\textcolor{red}{[SL: Here, there are conversion problems. The proof below also does not seem to work.]}
Next, we treat the $ \II $-term, where again no split is necessary.
\begin{align}
	...
	&\leq \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell}}} 
		...
		\norm{(\NN+1)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_*} \sum\limits_{r \in L_\ell}
		\left( \sum\limits_{\ell_1\in \Z^3_*} \sum_{s \in L_\ell}
		\mathds{1}_{L_{\ell_1}}(r)
		|K^{m-j}(\ell)_{r,q}|^2
		|K^{j}(\ell)_{q,s}|^2
		|K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( 
		\sum\limits_{\ell_1\in \Z^3_*} \sum_{s \in L_\ell}
	...
	&\quad \times \left( 
		\sum\limits_{\ell_1\in \Z^3_*} \sum_{s \in L_\ell}
		\norm{a_{r-\ell_1} a_{s-\ell} a_s (\NN+1)^{-\alpha} \psi}^2
		\right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_*}
		\norm{K^j(\ell)}_{\max,2}
		\norm{K^{m-j}(\ell)}_{\max,2}
		\left( \sum\limits_{\ell_1 \in \Z^3_*}
		\sum\limits_{r \in L_{\ell_1}}
		|K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12}
		\norm{(\NN+1)^{1-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
\end{align}
\textcolor{red}{[SL: This scales like $ k_{\F}^{\frac 32} $, which is too weak.]}
Finally, for the $ \III $-term,
\begin{align}
