...
\begin{lemma}\label{lem:estQ2}
    ...
    \begin{align}
        |\eva{\Psi,Q_1(A)\Psi}| &\leq 2\sum\limits_{\ell\in \Z^3_*}\norm{A(\ell)}_{\HS}\eva{\Psi,\mathcal{N} \Psi}\label{eq:Q1est}\\
        |\eva{\Psi,Q_2(A)\Psi}| &\leq 2\sum\limits_{\ell\in \Z^3_*}\norm{A(\ell)}_{\HS}\eva{\Psi,(\mathcal{N}+1) \Psi}\label{eq:Q2est}
    \end{align}
    for all $\Psi \in \HH_N$.
\end{lemma}
\begin{proof}
...
\end{equation}
\end{lemma}
and for any fixed $ s \in \Z^3 $, we have \textcolor{red}{[SL: set $ K(\ell)_{r,s} = 0 $ if $ r $ or $ s $ is outside $ L_\ell $.]}
\begin{equation} \label{eq:Krr_bound}
	\left( \sum_{r \in L_\ell} |K(\ell)^m_{r,r+s}|^2 \right)^{\frac 12}
	\le (C \hat{V}(\ell))^m k_{\F}^{-\frac 12} \;, \qquad
	\sum_{r \in L_\ell} |K(\ell)^m_{r,s}|
	\le (C \hat{V}(\ell))^m \;.
\end{equation}
\begin{proof}
...
Finally, \eqref{eq:Krr_bound} is proven like the bound for $ \norm{K(\ell)^m}_{\max,2} $:
\begin{equation}
\begin{aligned}
	\sum_{r \in L_\ell} |K(\ell)^m_{r,r+s}|^2
	&\le \sum_{r \in L_\ell} (C \hat{V}(\ell))^{2m} k_{\F}^{-2} (\lambda_{\ell,r} + \lambda_{\ell,r+s})^{-2}
	\le (C \hat{V}(\ell))^{2m} k_{\F}^{-2} \sum_{r \in L_\ell} \lambda_{\ell,r}^{-1}
	\le (C \hat{V}(\ell))^{2m} k_{\F} \;, \\
	\sum_{r \in L_\ell} |K(\ell)^m_{r,s}|
	&\le \sum_{r \in L_\ell} (C \hat{V}(\ell))^m k_{\F}^{-1} \lambda_{\ell,r}^{-1}
	\le (C \hat{V}(\ell))^m \;.
\end{aligned}
\end{equation}
\end{proof}
...
\begin{lemma}[Bound on nested anti-commutator]\label{lem:multicommest}
    ...
    \begin{equation}
    \sum\limits_{\ell \in \Z^3_*}\norm{\Theta^{n}_K(A)(\ell)}_{\HS}\leq    \sum\limits_{\ell \in \Z^3_*} 2^{n}\norm{K(\ell)}^{n}_{\mathrm{op}}\norm{A(\ell)}_{\HS}
    \end{equation}
...
\begin{proof}
    We begin with 
    \begin{align}
        \sum\limits_{\ell \in \Z^3_*}\norm{\Theta^{n}_K(A)(\ell)}_{\HS} = \sum\limits_{\ell \in \Z^3_*}\norm{\left\{K(\ell),\Theta^{n-1}_K(A)(\ell)\right\}}_{\HS} &=  \sum\limits_{\ell \in \Z^3_*}\norm{K(\ell)\Theta^{n-1}_K(A)(\ell) +\Theta^{n-1}_K(A)(\ell) K(\ell) }_{\HS}\nonumber\\
...
\begin{proof}
    ...
    \begin{equation}
      \mathrm{L.H.S.\,of\,} (\ref{eq:headest})   
        \leq  \abs{2\bint\limits_{\Delta^{n}_1}\di^n\underline{\lambda} \norm{\Theta^{n+1}_{K}(P^q)}_{\HS} \eva{\Omega,\Big( T^*_{\lambda_n}(\NN +1) T_{\lambda_n}\Big)\Omega}} 
    \end{equation}
    Then using Lemma \ref{lem:multicommest} we get
    \begin{equation}
        \leq  \abs{2\bint\limits_{\Delta^{n}_1}\di^n\underline{\lambda}\, 2^{n+1} \norm{K(\ell)}^{n+1}_{\mathrm{op}} \norm{(P^q)}_{\HS} \eva{\Omega, \Big( T^*_{\lambda_n} (\NN +1) T_{\lambda_n}\Big)\Omega}}
    \end{equation}
    Here we observe that $\norm{P^q}_{\HS} = \frac{1}{\sqrt{2}}$, and then we use the Gr\"onwall estimate from Lemma \ref{lem:gronNest} to have
    \begin{equation}
...
The aim of this subsection is to prove the following bound.
 
\begin{proposition}[$E_{Q_1}$] \label{prop:estEQ1}
Let $q \in \Z^3_*$ and $\psi \in \HH_N$. Then, for any $ S \subset \Z^3_* $, we have the estimate
\begin{align*}
     \abs{\eva{\psi,E_{Q_1} \psi }}
     &\leq C k_{\F} \Xi^{\half}
    		\left( k_{\F}^{-\frac 12} \sum_{\ell \in S} \hat{V}(\ell) + \left( \sum_{\ell \in \Z^3_* \setminus S} \hat{V}(\ell)^2 \right)^{\frac 12} \right)
    		\left( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \right)^{\frac 12}
    		\norm{(\NN+5)^2 \psi}^2 \;. \numberthis \label{eq:estEQ1}
\end{align*}
\end{proposition}

\begin{lemma}[Bounding $E_{Q_1}^{1,1}$] \label{lem:estEQ111}
Let $q \in \Z^3_*$ and $\psi \in \HH_N$. Then, for any $ S \subset \Z^3_* $, we have the estimate
\begin{align*}
...
\begin{align*}
	&\abs{\eva{\psi,\left(E^{\,1,1}_{Q_1}+\mathrm{h.c.}\right) \psi }}
	= \abs{\eva{\psi,2\mathrm{Re}\left(E^{\,1,1}_{Q_1}\right) \psi }} 
	\leq 4\abs{\eva{\psi, E^{\,1,1}_{Q_1} \psi }} \\
	...
	&\leq 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\s_1\in L_{\ell_1}}} \eva{\psi, K^m(\ell )_{r,q} K(\ell_1)_{r,s_1} a^*_{r-\ell_1} b^*_{q}(\ell) b^*_{-s_1}(-\ell_1) a_{r-\ell} \psi }} \\
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{j=1}^{m-1} {{m}\choose j} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell},s_1\in L_{\ell_1}}}  \eva{\psi,  K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q,s} K(\ell_1)_{r,s_1} a^*_{r-\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_{r-\ell} \psi }} \\ 
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \mathds{1}_{L_{\ell_1}}(q) \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \eva{\psi, K^m(\ell)_{q,s}K(\ell_1)_{q,s_1}
	a^*_{q-\ell_1}b^*_{s}(\ell) b^*_{-s_1}(-\ell_1)a_{q-\ell} \psi }} \\ 
	&=: 4\abs{\sum_{\ell \in \Z^3_*} \I_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \III_\ell } \;. \numberthis \label{eq:split_EQ_111}
\end{align*}
First, we consider the term $ \I $ and split the sum $ \sum_{\ell} $ according to whether $ \ell $ are in $ S $ or not. In any case, we then successively bound annihilation operators against $ \mathcal{N} = \sum_{p \in \Z^3} a_p^* a_p $, also using Lemma \ref{lem:pairest}. To balance the powers of $ \NN $, we use $I = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for some $\alpha \in \R$ to be fixed later. Further, we use the pull-through formula (Lemma \ref{lem:commNab}) and the Cauchy--Schwarz inequality. First,
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_* \setminus S} \I_\ell } \nonumber\\
    &\leq \abs{ \sum_{\ell \in \Z^3_* \setminus S} \sum\limits_{r \in L_\ell} \eva{ b_{q}(\ell)
    		\sum\limits_{\ell_1 \in \Z^3_*} \sum\limits_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell_1} \psi, 
    		 K^m_{r,q}(\ell) a_{r-\ell} \psi }}\nonumber\\
    &\leq \left( \sum_{\ell \in \Z^3_* \setminus S} \sum_{r \in L_\ell}
    		\norm{a_{q-\ell} a_q \sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell_1} (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	...
    	&\leq \left( \sum_{r \in \Z^3_*}
    		\norm{\sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell_1} a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2
    		\norm{ (\NN+5)^{\alpha}\psi }^2 \right)^{\frac 12} \nonumber\\
    &\leq \left( \sum_{r \in \Z^3_*} \left(  \sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} |K(\ell_1)_{r,s_1}|^2 \right)
    		\sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} 
    		\norm{ a_{r-\ell_1} a_{-s_1+\ell_1} a_{-s_1}  a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2 \right)^{\frac 12}
    		\norm{ (\NN+5)^{\alpha}\psi }  \nonumber\\
    	&\leq \left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
    		\left(
    		\sum_{s_1 \in \Z^3_*} \sum\limits_{\ell_1 \in \Z^3_*} \sum_{r \in \Z^3_*}
    		\norm{ a_{r-\ell_1} a_{-s_1+\ell_1} a_{-s_1}  a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2 \right)^{\frac 12}
    		\norm{ (\NN+5)^{\alpha}\psi }  \nonumber\\
    	&\leq \left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
    ...
    &\leq \left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
    		...
    		\norm{ (\NN+5)^2 \psi } \;,
\end{align}
where we set $ \alpha = 2 $. With Lemma~\ref{lem:K_bounds} and $ |\hat{V}(\ell)| \le C $, we conclude now
\begin{equation}
	\abs{\sum_{\ell \in \Z^3_* \setminus S} \I_\ell }
	\le C k_{\F}^{-1} \Xi^{\frac 12}
		...
		\norm{(\NN+5)^2 \psi} \;.
\end{equation}
The bound for $ \ell \in S $ is similar.
\begin{align}
	\abs{\sum_{\ell \in S} \I_\ell }
	&\leq \sum_{\ell \in S} \abs{ \sum\limits_{r \in L_\ell} \eva{ b_{q}(\ell)
    		\sum\limits_{\ell_1 \in \Z^3_*}  \sum\limits_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell_1} \psi, 
    		 K^m_{r,q}(\ell) a_{r-\ell} \psi }}\nonumber\\
    	&\leq \sum_{\ell \in S} \left( \sum_{r \in \Z^3_*}
    		\norm{\sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell_1} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \norm{K^m(\ell)}_{\max}^2
    		\sum_{r \in L_\ell} \norm{ a_{r-\ell} (\NN+5)^{\alpha}\psi }^2 \right)^{\frac 12} \nonumber\\
    	&\leq \sum_{\ell \in S} \norm{K^m(\ell)}_{\max}
    	\left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
    		\norm{ a_q (\NN+1)^{\frac 32-\alpha} \psi}
    		\norm{ (\NN+5)^{\frac 12+\alpha}\psi }  \nonumber\\
    	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
    		\left( k_{\F}^{-\frac 12} \sum_{\ell \in S} \hat{V}(\ell) \right)
    		\left( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \right)^{\frac 12}
    		\norm{(\NN+5)^2 \psi} \;,
\end{align}
where we used $ \alpha = \frac 32 $ and directly applied Lemma~\ref{lem:K_bounds} in the last line.\\
For the $ \II $-term, there is no need to split for $ \ell \in S $ or $ \ell \notin S $, but we directly estimate
\begin{align}
	&\abs{\sum\limits_{\ell, \ell_1 \in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell},s_1\in L_{\ell_1}}}  \eva{\psi, K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q	,s} K(\ell_1)_{r,s_1} a^*_{r-\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_{r-\ell} \psi}} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \left( \sum_{r \in L_\ell} 
		\norm{ \sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \mathds{1}_{L_{\ell_1}}(r) K^{j}(\ell)_{q	,s} K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) b_{s}(\ell) a_{r-\ell_1} \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum_{r \in L_\ell} \abs{K^{m-j}(\ell)_{r,q}}^2 \norm{a_{r-\ell} \psi}^2 \right)^{\frac 12} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \left( \sum_{r \in L_\ell} 
		\sum_{\ell_1' \in \Z^3_*} \sum_{s' \in L_{\ell},s_1'\in L_{\ell_1'}} |K^j(\ell)_{q,s'}|^2 |K(\ell_1')_{r,s_1'}|^2
		\sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{ b_{-s_1}(-\ell_1) b_{s}(\ell) a_{r-\ell_1} \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \norm{K^{m-j}(\ell)_{r,q}}_{\max,2} \Xi^{\frac 12} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \norm{K^j(\ell)}_{\max,2}
		\left( \sum_{\ell_1' \in \Z^3_*}	\norm{K(\ell_1')}_{\max,2}^2 \right)^{\frac 12}
		\left( \sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \sum_{r \in L_\ell}
		\norm{ a_{r-\ell_1} a_{-s_1+\ell_1} a_{-s_1} a_{s-\ell} a_s \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \norm{K^{m-j}(\ell)_{r,q}}_{\max,2} \Xi^{\frac 12} \nonumber\\
	& \leq \Xi^{\frac 12} \sum_{\ell \in \Z^3_*}
		\norm{K^j(\ell)}_{\max,2}
		\norm{K^{m-j}(\ell)}_{\max,2}
		\left( \sum_{\ell_1 \in \Z^3_*}	\norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
		\norm{\NN^2 \psi} \;.
\end{align}
With Lemma~\ref{lem:K_bounds} and $ |\hat{V}(\ell)| \le C $, we conclude
\begin{equation}
	\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
	\leq C k_{\F}^{-\frac 32} \Xi^{\frac 12}
		\left( \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \right)^{\frac 32}
		\norm{\NN^2 \psi} \;.
\end{equation}
Finally, for the $ \III $-term,
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_*} \III_\ell } \nonumber\\
	&\leq \norm{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \mathds{1}_{L_{\ell_1}}(q) \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} K^m(\ell)_{q,s}K(\ell_1)_{q,s_1}
	b_{-s_1}(-\ell_1) b_{s}(\ell) a_{q-\ell_1} \psi}
	\norm{a_{q-\ell} \psi} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} |K^m(\ell)_{q,s}|^2 |K(\ell_1)_{q,s_1}|^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{a_{-s_1} a_{-s_1+\ell_1} a_s a_{s-\ell} a_{q-\ell_1} \psi}^2 \right)^{\frac 12}
	\Xi^{\frac 12} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \norm{K^m(\ell)}_{\max,2}^2 \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{a_{-s_1} a_{-s_1+\ell_1} a_s a_{s-\ell} \psi}^2 \right)^{\frac 12}
	\Xi^{\frac 12} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \norm{K^m(\ell)}_{\max,2}^2 \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
		\norm{\NN^2 \psi}
	\Xi^{\frac 12} \nonumber\\
	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
		\sum_{\ell \in \Z^3_*} \hat{V}(\ell)^2
		\norm{\NN^2 \psi} \;.
\end{align}
\textcolor{red}{[SL: I did not manage to control do the conversion into $ \sum_\ell \hat{V}(\ell) $. Is it even possible? If not, we may need to impose a lower bound on $ \lambda_{\ell,q} $.]}
\end{proof}

The bound for $ E_{Q_1}^{2,1} $ is effectively analogous.

\begin{lemma}[Bounding $E_{Q_1}^{2,1}$] \label{lem:estEQ121}
Let $q \in \Z^3_*$ and $\psi \in \HH_N$. Then, for any $ S \subset \Z^3_* $, we have the estimate
\begin{align}
     ...
     &\leq C k_{\F} \Xi^{\half}
    		\left( k_{\F}^{-\frac 12} \sum_{\ell \in S} \hat{V}(\ell) + \left( \sum_{\ell \in \Z^3_* \setminus S} \hat{V}(\ell)^2 \right)^{\frac 12} \right)
    		\left( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \right)^{\frac 12}
    		\norm{(\NN+5)^2 \psi}^2 \;. \label{eq:estEQ121}\\
\end{align}
\end{lemma}
\begin{proof} Recall the definition \eqref{eq:???} of $ E^{\,2,1}_{Q_1} $, where by \eqref{eq:decomptheta}, the multi-commutator $ \Theta^m_K(P^q) $ has three contributions. Thus,
\begin{align*}
	&\abs{\eva{\psi,\left(E^{\,2,1}_{Q_1}+\mathrm{h.c.}\right) \psi }}
	= \abs{\eva{\psi,2\mathrm{Re}\left(E^{\,2,1}_{Q_1}\right) \psi }}
	\leq 4\abs{\eva{\psi, E^{\,2,1}_{Q_1} \psi }} \\
	&=4\abs{\eva{\psi, \sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r\in (L_{\ell}-\ell) \cap (L_{\ell_1}-\ell_1)\\  s \in L_{\ell}, s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)(\ell)_{r+\ell,s} K(\ell_1)_{r+\ell_1,s_1} a^*_{r+\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_{r+\ell} \psi }} \\
	&=4\abs{\eva{\psi, \sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r\in L_{\ell} \cap (L_{\ell_1}+\ell-\ell_1)\\  s \in L_{\ell}, s_1\in L_{\ell_1}}} \Theta^m_{K}(P^q)(\ell)_{r,s} K(\ell_1)_{r-\ell+\ell_1,s_1} a^*_{r-\ell+\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_r \psi }} \\
	&\leq 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap (L_{\ell_1}+\ell-\ell_1)\\s_1\in L_{\ell_1}}} \eva{\psi, K^m(\ell )_{r,q} K(\ell_1)_{r-\ell+\ell_1,s_1} a^*_{r-\ell+\ell_1} b^*_{q}(\ell) b^*_{-s_1}(-\ell_1) a_r \psi }} \\
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{j=1}^{m-1} {{m}\choose j} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap (L_{\ell_1}+\ell-\ell_1)\\ s \in L_{\ell},s_1\in L_{\ell_1}}}  \eva{\psi,  K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q,s} K(\ell_1)_{r-\ell+\ell_1,s_1} a^*_{r-\ell+\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_r \psi }} \\ 
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(q) \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \eva{\psi, K^m(\ell)_{q,s}K(\ell_1)_{q,s_1}
	a^*_{q-\ell+\ell_1}b^*_{s}(\ell) b^*_{-s_1}(-\ell_1)a_q \psi }} \\ 
	&=: 4\abs{\sum_{\ell \in \Z^3_*} \I_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \III_\ell } \;. \numberthis \label{eq:split_EQ_121}
\end{align*}
As in Lemma \ref{lem:estEQ111}, we bound $ \I $: \textcolor{red}{[SL: Maybe, we should bound $ E_{Q_1}^{2,1} $ first, as it is slightly more difficult than $ E_{Q_1}^{1,1} $]}
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_* \setminus S} \I_\ell } \nonumber\\
    &\leq \abs{ \sum_{\ell \in \Z^3_* \setminus S} \sum\limits_{r \in L_\ell} \eva{ b_{q}(\ell)
    		\sum\limits_{\ell_1 \in \Z^3_*} \sum\limits_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K(\ell_1)_{r-\ell+\ell_1,s_1} b_{-s_1}(-\ell_1) a_{r-\ell+\ell_1} \psi, 
    		 K^m_{r,q}(\ell) a_r \psi }}\nonumber\\
    &\leq \left( \sum_{\ell \in \Z^3_* \setminus S} \sum_{r \in L_\ell}
    		\norm{a_{q-\ell} a_q \sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K(\ell_1)_{r-\ell+\ell_1,s_1} b_{-s_1}(-\ell_1) a_{r-\ell+\ell_1} (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \sum\limits_{r \in L_\ell} 
    		|K^m_{r,q}(\ell)|^2 \norm{  a_r (\NN+5)^{\alpha}\psi }^2 \right)^{\frac 12} \nonumber\\
    	&\leq \left( \sum_{\ell \in \Z^3_* \setminus S}  \sum_{r \in \Z^3_*}
    		\norm{a_{q-\ell} \sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K(\ell_1)_{r,s_1} b_{-s_1}(-\ell_1) a_{r-\ell+\ell_1} a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2
    		\norm{ (\NN+5)^{\alpha}\psi }^2 \right)^{\frac 12} \nonumber\\
    &\leq \left( \sum_{\ell \in \Z^3_* \setminus S}  \sum_{r \in \Z^3_*} \left(  \sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} |K(\ell_1)_{r,s_1}|^2 \right)
    		\sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} 
    		\norm{ a_{r-\ell+\ell_1} a_{q-\ell} a_{-s_1+\ell_1} a_{-s_1}  a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2 \right)^{\frac 12}
    		\norm{ (\NN+5)^{\alpha}\psi }  \nonumber\\
    	&\leq \left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
    		\left(
    		\sum_{s_1 \in \Z^3_*} \sum\limits_{\ell_1 \in \Z^3_*} \sum_{\ell \in \Z^3_*} \sum_{r \in \Z^3_*}
    		\norm{ a_{r-\ell+\ell_1} a_{q-\ell} a_{-s_1+\ell_1} a_{-s_1}  a_q (\NN+1)^{\frac 12-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \sum_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2 \right)^{\frac 12}
    		\norm{ (\NN+5)^{\alpha}\psi }  \nonumber\\
    	&\leq \left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
...
\begin{align}
	...
	&\leq \sum_{\ell \in S} \abs{ \sum\limits_{r \in L_\ell} \eva{ b_{q}(\ell)
    		\sum\limits_{\ell_1 \in \Z^3_*}  \sum\limits_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K(\ell_1)_{r-\ell+\ell_1,s_1} b_{-s_1}(-\ell_1) a_{r-\ell+\ell_1} \psi, 
    		 K^m_{r,q}(\ell) a_r \psi }}\nonumber\\
    	&\leq \sum_{\ell \in S} \left( \sum_{r \in \Z^3_*}
    		\norm{\sum\limits_{\ell_1 \in \Z^3_*} \sum_{s_1 \in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K(\ell_1)_{r-\ell+\ell_1,s_1} b_{-s_1}(-\ell_1) a_{r-\ell+\ell_1} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12} \times \nonumber\\
    	&\quad \times \left( \norm{K^m(\ell)}_{\max}^2
    		\sum_{r \in L_\ell} \norm{ a_r (\NN+5)^{\alpha}\psi }^2 \right)^{\frac 12} \nonumber\\
    	&\leq \sum_{\ell \in S} \norm{K^m(\ell)}_{\max}
    	\left( \sum_{\ell_1 \in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
...
For the $ \II $-term, there is no need to split for $ \ell \in S $ or $ \ell \notin S $, but we directly estimate
\begin{align}
	&\abs{\sum\limits_{\ell, \ell_1 \in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap (L_{\ell_1}+\ell-\ell_1)\\ s \in L_{\ell},s_1\in L_{\ell_1}}}  \eva{\psi, K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q,s} K(\ell_1)_{r-\ell+\ell_1,s_1} a^*_{r-\ell+\ell_1} b^*_{s}(\ell) b^*_{-s_1}(-\ell_1) a_r \psi}} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \left( \sum_{r \in L_\ell} 
		\norm{ \sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(r) K^{j}(\ell)_{q,s} K(\ell_1)_{r-\ell+\ell_1',s_1} b_{-s_1}(-\ell_1) b_{s}(\ell) a_{r-\ell+\ell_1} \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum_{r \in L_\ell} \abs{K^{m-j}(\ell)_{r,q}}^2 \norm{a_r \psi}^2 \right)^{\frac 12} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \left( \sum_{r \in L_\ell} 
		\sum_{\ell_1' \in \Z^3_*} \sum_{s' \in L_{\ell},s_1'\in L_{\ell_1'}} |K^j(\ell)_{q,s'}|^2 |K(\ell_1')_{r,s_1'}|^2
		\sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{ b_{-s_1}(-\ell_1) b_{s}(\ell) a_{r-\ell+\ell_1} \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \norm{K^{m-j}(\ell)_{r,q}}_{\max,2} \Xi^{\frac 12} \nonumber\\
	&\leq \sum_{\ell \in \Z^3_*} \norm{K^j(\ell)}_{\max,2}
		\left( \sum_{\ell_1' \in \Z^3_*}	\norm{K(\ell_1')}_{\max,2}^2 \right)^{\frac 12}
		\left( \sum_{\ell_1 \in \Z^3_*} \sum_{s \in L_{\ell},s_1\in L_{\ell_1}} \sum_{r \in L_\ell}
		\norm{ a_{r-\ell+\ell_1} a_{-s_1+\ell_1} a_{-s_1} a_{s-\ell} a_s \psi }^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \norm{K^{m-j}(\ell)_{r,q}}_{\max,2} \Xi^{\frac 12} \nonumber\\
	& \leq \Xi^{\frac 12} \sum_{\ell \in \Z^3_*}
		\norm{K^j(\ell)}_{\max,2}
		\norm{K^{m-j}(\ell)}_{\max,2}
		...
		\norm{\NN^2 \psi} \;.
\end{align}
Again, with Lemma~\ref{lem:K_bounds} and $ |\hat{V}(\ell)| \le C $, we conclude
\begin{equation}
	\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
	\leq C k_{\F}^{-\frac 32} \Xi^{\frac 12}
		\left( \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \right)^{\frac 32}
...
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_*} \III_\ell } \nonumber\\
	&\leq \norm{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \mathds{1}_{L_{\ell_1}+\ell-\ell_1}(q) \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} K^m(\ell)_{q,s}K(\ell_1)_{q-\ell+\ell_1,s_1}
	b_{-s_1}(-\ell_1) b_{s}(\ell) a_{q-\ell+\ell_1} \psi}
	\norm{a_q \psi} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} |K^m(\ell)_{q,s}|^2 |K(\ell_1)_{q-\ell+\ell_1,s_1}|^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{a_{-s_1} a_{-s_1+\ell_1} a_s a_{s-\ell} a_{q-\ell+\ell_1} \psi}^2 \right)^{\frac 12}
	\Xi^{\frac 12} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \norm{K^m(\ell)}_{\max,2}^2 \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{s \in L_{\ell},s_1\in L_{\ell_1}} \norm{a_{-s_1+\ell_1} a_{-s_1} a_{s-\ell} a_s \psi}^2 \right)^{\frac 12}
	\Xi^{\frac 12} \nonumber\\
	&\leq \left( \sum\limits_{\ell, \ell_1\in \Z^3_*} \norm{K^m(\ell)}_{\max,2}^2 \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12}
		\norm{\NN^2 \psi}
	\Xi^{\frac 12} \nonumber\\
	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
		...
		\norm{\NN^2 \psi} \;.
\end{align}
\textcolor{red}{[SL: Here, we have the same problem.]}
\end{proof}


\begin{lemma}[Bounding $E_{Q_1}^{1,2}$] \label{lem:estEQ112}
Let $q \in \Z^3_*$ and $\psi \in \HH_N$. Then, for any $ S \subset \Z^3_* $, we have the estimate
\begin{align}
     \abs{\eva{\psi,\left(E^{\,1,2}_{Q_1}+\mathrm{h.c.}\right) \psi }}\nonumber
     &\leq C k_{\F} \Xi^{\half}
    		\left( k_{\F}^{-\frac 12} \sum_{\ell \in S} \hat{V}(\ell) + \left( \sum_{\ell \in \Z^3_* \setminus S} \hat{V}(\ell)^2 \right)^{\frac 12} \right)
    		\left( \sum_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1)^2 \right)^{\frac 12}
    		\norm{(\NN+5)^2 \psi}^2 \;. \label{eq:estEQ112}\\
\end{align}
\end{lemma}
\begin{proof} Recall the definition \eqref{eq:???} of $ E^{\,1,2}_{Q_1} $, where by \eqref{eq:decomptheta}, the multi-commutator $ \Theta^m_K(P^q) $ has three contributions. Thus,
\begin{align*}
	&\abs{\eva{\psi,\left(E^{\,1,2}_{Q_1}+\mathrm{h.c.}\right) \psi }}
	= \abs{\eva{\psi,2\mathrm{Re}\left(E^{\,1,2}_{Q_1}\right) \psi }} 
	\leq 4\abs{\eva{\psi, E^{\,1,2}_{Q_1} \psi }} \\
	&\leq 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \sum\limits_{r\in L_{\ell} \cap L_{\ell_1}} \eva{\psi, K^m(\ell)_{r,q} K(\ell_1)_{r,-r+\ell+\ell_1} a^*_{r-\ell_1} b^*_{q}(\ell) a^*_{r-\ell-\ell_1}  \psi }} \\
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{j=1}^{m-1} {{m}\choose j} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell}}} \eva{\psi,  K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q,s} K(\ell_1)_{r,-r+\ell+\ell_1} a^*_{r-\ell_1} b^*_{s}(\ell) a^*_{r-\ell-\ell_1} \psi }} \\ 
	&\quad + 4\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \mathds{1}_{L_{\ell_1}}(q) \mathds{1}_{L_{\ell_1}}(-q+\ell+\ell_1) \sum\limits_{s \in L_{\ell}} \eva{\psi, K^m(\ell)_{q,s}K(\ell_1)_{q,-q+\ell+\ell_1}
	a^*_{q-\ell_1}b^*_{s}(\ell) a^*_{q-\ell-\ell_1} \psi }} \\ 
	&=: 4\abs{\sum_{\ell \in \Z^3_*} \I_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
		+ 4\abs{\sum_{\ell \in \Z^3_*} \III_\ell } \;. \numberthis \label{eq:split_EQ_112}
\end{align*}
For the $ \I $-term, we again split according to whether $ \ell $ is in $ S $, and use the decomposition $ \I = (\NN+1)^{\alpha} (\NN+1)^{-\alpha} $, which is valid for any $ \alpha \in \mathbb{R} $:
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_* \setminus S} \I_\ell } \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \sum\limits_{r\in L_{\ell} \cap L_{\ell_1}}
		\norm{ K^m(\ell)_{r,q} K(\ell_1)_{r,-r+\ell+\ell_1} a_{r-\ell_1} a_{r-\ell-\ell_1} b_{q}(\ell) (\NN+1)^{-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{r\in \Z^3} 
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \mathds{1}_{L_\ell \cap L_{\ell_1}}(r) |K^m(\ell)_{r,q}|^2 |K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \norm{ a_{r-\ell_1} a_{r-\ell-\ell_1} a_{q-\ell} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{r\in \Z^3} 
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S} \mathds{1}_{L_\ell}(r) |K^m(\ell)_{r,q}|^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell_1\in \Z^3_*} \mathds{1}_{L_{\ell_1}}(r) |K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \norm{ a_{r-\ell_1} a_{q-\ell} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq 
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S} \sum\limits_{r\in _\ell}|K^m(\ell)_{r,q}|^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell_1\in \Z^3_*} \sum\limits_{r\in L_{\ell_1}} |K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \norm{ a_q (\NN+1)^{1-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S} \hat{V}(\ell)^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell_1\in \Z^3_*} \hat{V}(\ell_1)^2 \right)^{\frac 12} 
		\norm{(\NN+5) \psi} \;, \nonumber\\
\end{align}
where we fixed $ \alpha = 1 $ and used Lemma~\ref{lem:K_bounds}. On the other hand,
\begin{align}
	&\abs{\sum_{\ell \in S} \I_\ell } \nonumber\\
	&\leq \sum_{\ell \in S} \sum\limits_{\ell_1\in \Z^3_*} \sum\limits_{r\in L_{\ell} \cap L_{\ell_1}}
		\norm{ K^m(\ell)_{r,q} K(\ell_1)_{r,-r+\ell+\ell_1} a_{r-\ell_1} a_{r-\ell-\ell_1} b_{q}(\ell) (\NN+1)^{-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum_{\ell \in S} \left( \sum\limits_{\ell_1\in \Z^3_*} \sum\limits_{r\in L_{\ell} \cap L_{\ell_1}}
		|K^m(\ell)_{r,q}|^2 |K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell_1, r \in \Z^3_*}
		\norm{a_{q-\ell} a_{r-\ell_1} a_{r-\ell-\ell_1} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum_{\ell \in S}
		\norm{K^m(\ell)}_{\max} \left( \sum\limits_{\ell_1\in \Z^3_*} \norm{K(\ell_1)}_{\max,2}^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell_1, r \in \Z^3_*}
		\norm{a_{r-\ell_1} a_{r-\ell-\ell_1} a_q (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \;. \nonumber\\
\end{align}
\textcolor{red}{[SL: Here, there are conversion problems.]}
Next, we treat the $ \II $-term. Again, no split is necessary.
\begin{align}
	&\abs{\sum\limits_{\ell, \ell_1\in \Z^3_*} \mathds{1}_{L_{\ell}}(q) \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell}}} \eva{\psi,  K^{m-j}(\ell)_{r,q} K^{j}(\ell)_{q,s} K(\ell_1)_{r,-r+\ell+\ell_1} a^*_{r-\ell_1} b^*_{s}(\ell) a^*_{r-\ell-\ell_1} \psi }} \nonumber\\
	&\leq \sum\limits_{\ell, \ell_1\in \Z^3_*} \sum\limits_{\substack{r\in L_{\ell} \cap L_{\ell_1}\\ s \in L_{\ell}}} 
		|K^{m-j}(\ell)_{r,q}|
		|K^{j}(\ell)_{q,s}|
		|K(\ell_1)_{r,-r+\ell+\ell_1}|
		\norm{a_{r-\ell_1} a_{r-\ell-\ell_1} a_s a_{s-\ell} (\NN+1)^{-\alpha} \psi}
		\norm{(\NN+1)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_*} \sum\limits_{r,s \in L_\ell}
		|K^{j}(\ell)_{q,s}|
		\left( \sum\limits_{\ell_1\in \Z^3_*}
		\mathds{1}_{L_{\ell_1}}(r)
		|K^{m-j}(\ell)_{r,q}|^2
		|K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( 
		\sum\limits_{\ell_1\in \Z^3_*}
		\norm{a_{r-\ell_1} a_{r-\ell-\ell_1} a_s a_{s-\ell} (\NN+1)^{-\alpha} \psi}^2
		\right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_*} \sum\limits_{s \in L_\ell}
		|K^{j}(\ell)_{q,s}|
	...
	&\quad \times \left( 
		\sum\limits_{\ell_1\in \Z^3_*}
		\norm{a_{r-\ell_1} a_s (\NN+1)^{-\alpha} \psi}^2
		\right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_*} \sum\limits_{s \in L_\ell}
		|K^{j}(\ell)_{q,s}|
		\left( \sum\limits_{r \in L_\ell}
		|K^{m-j}(\ell)_{r,q}|^2 \right)^{\frac 12}
		\left( \sum\limits_{\ell_1 \in \Z^3_*}
		\sum\limits_{r \in L_\ell \cap L_{\ell_1}}
		|K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times
		\norm{a_s (\NN+1)^{\frac 12-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \Xi^{\frac 12} \sum\limits_{\ell \in \Z^3_*} \sum\limits_{s \in L_\ell}
		|K^{j}(\ell)_{q,s}|
		\norm{K^{m-j}(\ell)}_{\max,2}
		\left( \sum\limits_{\ell_1 \in \Z^3_*}
		\sum\limits_{r \in L_\ell \cap L_{\ell_1}}
		|K(\ell_1)_{r,-r+\ell+\ell_1}|^2 \right)^{\frac 12}
		\norm{(\NN+5)^{\frac 12} \psi} \nonumber\\
	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
		\left( \sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \right)^{\frac 32}
		\norm{(\NN+5)^{\frac 12} \psi} \;, \nonumber\\
\end{align}
where we chose $ \alpha = \frac 12 $ and used~\eqref{eq:Krr_bound} in the last line. We conclude
\begin{equation}
	\abs{\sum_{\ell \in \Z^3_*} \II_\ell }
	\leq C k_{\F}^{-1} \Xi^{\frac 12}
		\left( \sum_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \right)^{\frac 32}
...
Finally, for the $ \III $-term,
\begin{align}
	&\abs{\sum_{\ell \in \Z^3_* \setminus S} \III_\ell } \nonumber\\
	&\leq \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \sum\limits_{s\in L_{\ell}}
		|K^m(\ell)_{q,s}|
		|K(\ell_1)_{q,-q+\ell+\ell_1}|
		\norm{ a_{q-\ell_1} a_{q-\ell-\ell_1} b_s(\ell) (\NN+1)^{-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{s\in \Z^3}
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \mathds{1}_{L_\ell}(s)
		|K^m(\ell)_{q,s}|^2
		|K(\ell_1)_{q,-q+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \norm{ a_{q-\ell_1} a_{q-\ell-\ell_1} a_s a_{s-\ell} (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \sum\limits_{s\in \Z^3}
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \mathds{1}_{L_\ell}(s)
		|K^m(\ell)_{q,s}|^2
		|K(\ell_1)_{q,-q+\ell+\ell_1}|^2 \right)^{\frac 12} 
		\norm{ a_s (\NN+1)^{1-\alpha} \psi}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \Xi^{\frac 12} \sum\limits_{s\in \Z^3}
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \mathds{1}_{L_\ell}(s)
		|K^m(\ell)_{q,s}|^2
		|K(\ell_1)_{q,-q+\ell+\ell_1}|^2 \right)^{\frac 12}
		\norm{(\NN+5) \psi} \nonumber\\
\end{align}
\textcolor{red}{[SL: I don't see how to pull the $ \sum_s $ into the bracket to convert $ \sum_s |K^m(\ell)_{q,s}| \le C $, without going over from $ \sum_\ell \hat{V}(\ell)^2 $ to $ \sum_\ell \hat{V}(\ell) $.]}
%TODO Writing head
\end{proof}


\begin{proof}[Proof of Proposition \ref{prop:estEQ1}]
The estimate follows by putting together the results from Lemma \ref{lem:estEQ111}, \ref{lem:estEQ121} and \ref{lem:estEQ112}.
\end{proof}
...
\begin{lemma}[Bounding $E_{Q_2}^{1,2}$ and $E_{Q_2}^{2,2}$]
    For any $\psi \in \HH_N$, we have
    \begin{align}
    	\abs{\eva{\psi,\left(E^{\,1,2}_{Q_2}+\mathrm{h.c.}\right) \psi }}, 
    	\abs{\eva{\psi,\left(E^{\,2,2}_{Q_2}+\mathrm{h.c.}\right) \psi }}
    	&\leq C k_{\F}^{-1} \Xi
    		\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \;. \label{eq:estEQ212}
    \end{align}
\end{lemma}
\begin{proof}
...
\begin{align}
    	\eqref{eq:EQ2121}
    	&= \sum\limits_{\ell \in \Z^3_*} \mathds{1}_{L_{\ell}}(q) 
    		\sum\limits_{r\in L_{\ell}} \abs{\eva{ K(\ell)_{r,q} a_{r-\ell} \psi, K^m(\ell)_{r,q}  a_{r-\ell} \psi }}\nonumber\\
    	&\leq \sum\limits_{\ell \in \Z^3_*}
    		\sum\limits_{r\in L_{\ell}} |K(\ell)_{r,q}| |K^m(\ell)_{r,q}|
    		\norm{ a_{r-\ell} \psi}\norm{ a_{r-\ell} \psi }\nonumber\\
    	&\leq \Xi \sum\limits_{\ell \in \Z^3_*}
    		\left( \sum\limits_{r\in L_{\ell}} |K(\ell)_{r,q}|^2 \right)^{\frac 12}
    		\left( \sum\limits_{r\in L_{\ell}} |K^m(\ell)_{r,q}|^2 \right)^{\frac 12} \nonumber\\
    	&\leq \Xi
    		\sum\limits_{\ell \in \Z^3_*} \norm{K(\ell)}_{\max,2} \norm{K^m(\ell)}_{\max,2} \nonumber\\
    	&\leq C k_{\F}^{-1} \Xi
    		\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \;. \label{eq:estEQ2121}
\end{align}
Likewise, we estimate \eqref{eq:EQ2122} as
\begin{align}
     \eqref{eq:EQ2122}
    ...
    &\leq \Xi \sum\limits_{j=1}^{m-1} {{m}\choose j} \sum\limits_{\ell \in \Z^3_*}
    		\sum\limits_{r,s\in L_{\ell}} |K(\ell)_{r,s}| |K^{m-j}(\ell)_{r,q}| |K^{j}(\ell)_{q,s}| \nonumber\\
    	&\leq \Xi \sum\limits_{j=1}^{m-1} {{m}\choose j} \sum\limits_{\ell \in \Z^3_*}
    		\left( \sum\limits_{s\in L_{\ell}} \left( \sum\limits_{r\in L_{\ell}} |K(\ell)_{r,s}| |K^{m-j}(\ell)_{r,q}| \right)^2 \right)^{\frac 12}
    		\left( \sum\limits_{s\in L_{\ell}} |K^{j}(\ell)_{q,s}|^2 \right)^{\frac 12} \nonumber\\
    	&\leq \Xi \sum\limits_{j=1}^{m-1} {{m}\choose j} \sum\limits_{\ell \in \Z^3_*}
    		\left( \sum\limits_{r,s\in L_{\ell}} |K(\ell)_{r,s}|^2
    			\sum\limits_{r\in L_{\ell}} |K^{m-j}(\ell)_{r,q}|^2 \right)^{\frac 12}
    		\norm{K^{j}(\ell)}_{\max,2} \nonumber\\
    	&\leq \Xi \sum\limits_{j=1}^{m-1} {{m}\choose j} \sum\limits_{\ell \in \Z^3_*}
    		\norm{K(\ell)}_{\HS}
    		\norm{K^{m-j}(\ell)}_{\max,2}
    		\norm{K^{j}(\ell)}_{\max,2} \nonumber\\
    &\leq C k_{\F}^{-1} \Xi \sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \;, \nonumber\\ \label{eq:estEQ2122}
\end{align}
where we used in the last step that $ \sum_{\ell} \hat{V}(\ell)^3 \le C \sum_{\ell} \hat{V}(\ell)^2 $. Finally, we estimate \eqref{eq:EQ2123} just as \eqref{eq:EQ2121}
\begin{align}
\eqref{eq:EQ2123}
    	...
    	&\leq \sum\limits_{\ell \in \Z^3_*} \sum\limits_{ s \in L_{\ell}} |K(\ell)_{q,s}| |K^m(\ell)_{q,s}| \norm{ a_{q-\ell} \psi}\norm{ a_{q-\ell} \psi } \nonumber\\
    	&\leq C k_{\F}^{-1} \Xi
    		\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^2 \label{eq:estEQ2123}
\end{align} 
Then adding \eqref{eq:estEQ2121},\eqref{eq:estEQ2122} and \eqref{eq:estEQ2123}, we arrive at the bound for $ E^{\,1,2}_{Q_2} $ in \eqref{eq:estEQ212}. The bound for $ E^{\,2,2}_{Q_2} $ is analogous, with $ a_{r-\ell} $ replaced by $ a_{r+\ell} $.
\end{proof}
