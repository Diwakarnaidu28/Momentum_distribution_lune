...
Then we flip the $\ell_1$ momenta, i.e., $\ell_1 = -\ell_1$ and use the symmetry $K(\ell)_{p,q} = K(-\ell)_{-p,-q}$ which gives us
	\begin{align}
		...
		&=-\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r' \in  L_{\ell} \cap (L_{\ell_1}) \\ \, \cap\\ \phantom{r'\in}  (-L_{\ell_1}+\ell_1+\ell)}} \!\!\!\! \mathds{1}_{L_\ell}(q) K^{m}_{r',q}(\ell) K_{-r'+\ell_1+\ell,r'}(\ell_1)\Big( b^*_{q}(\ell) a^*_{r'-\ell_1-\ell}a^*_{r'-\ell_1}\Big)\nonumber\\
		&=\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r' \in  L_{\ell} \cap (L_{\ell_1}) \\ \, \cap\\ \phantom{r'\in}  (-L_{\ell_1}+\ell_1+\ell)}} \!\!\!\! \mathds{1}_{L_\ell}(q) K^{m}_{r',q}(\ell) K_{-r'+\ell_1+\ell,r'}(\ell_1)\Big( b^*_{q}(\ell) a^*_{r'-\ell_1} a^*_{r'-\ell_1-\ell}\Big)\label{eq:EQ122} 
	\end{align}
where in the last line we used the CAR to exchange the two creation operators. And we see that \eqref{eq:EQ112}=\eqref{eq:EQ122}.

One can similarly prove the statement for $i = 3$ using the same identifications as above. For $i = 5$, one only flips $\ell_1$ and uses the symmetry  $K(\ell)_{p,q} = K(-\ell)_{-p,-q}$ and the statement holds for $i=1,3,5$. 
\end{proof}
