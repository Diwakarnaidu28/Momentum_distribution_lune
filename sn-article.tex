...
\begin{align}
	...
	&\leq \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \sum\limits_{s\in L_{\ell}}
		...
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \sum\limits_{s\in L_\ell}
		|K^m(\ell)_{q,s}|^2
		|K(\ell_1)_{q,-q+\ell+\ell_1}|^2 \right)^{\frac 12} \times \nonumber\\
	&\quad \times \left( \sum\limits_{\ell \in \Z^3_* \setminus S, \ell_1\in \Z^3_*} \sum\limits_{s\in \Z^3} \norm{ a_{q-\ell_1} a_{q-\ell-\ell_1} a_s a_{s-\ell} (\NN+1)^{-\alpha} \psi}^2 \right)^{\frac 12}
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
\end{align}
%TODO Writing head
\end{proof}
