...
\begin{align}
    T_1^*\left(n_q+n_{-q}\right)T_1 &=\left(n_q+n_{-q}\right)+ \sum\limits_{\ell\in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{m=1}^{\floor{(n+1)/2}} \frac{\mathrm{Tr} \,\Theta^{(2m)}_K \big(P^q\big)}{(2m)!} + \sum\limits_{m=1}^{n+1} E_m(P^q) + Q_1\left( \sum\limits_{m=1}^{\floor{n/2}} \frac{\Theta^{2m}_{K}(P^q)}{(2m)!}\right)  \nonumber\\
    &\quad- Q_2\left(\sum\limits_{m=1}^{\floor{(n+1)/2}} \frac{\Theta^{2m-1}_{K}(P^q)} {(2m-1)!}\right) +\bint\limits_{\Delta^{n+1}} \di^n \underline{\lambda} (-1)^{n+1} \Big( T^*_{\lambda_{n+1}} Q_{\sigma(n)}(\Theta^{n+1}_{K}(P^q)) T_{\lambda_{n+1}}\Big)
\end{align}
where $E_m(P^q)$ is defined as
\begin{equation}\label{eq:errEm}
    E_m(P^q) \coloneq -\bint\limits_{\Delta^{m}} \di^m\underline{\lambda} T^*_{\lambda_m} E_{Q_{\sigma(m)}}\left(\Theta^{m}_{K}(P^q)\right)T_{\lambda_m} .
    \end{equation}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\quad-\sum\limits_{m=1}^{n+1} \bint\limits_{\Delta ^m}\di^m\underline{\lambda} \big(T^*_{\lambda_{m}}E_{Q_{\sigma(m-1)}} \Theta^{m}_K\big(P^q\big)T_{\lambda_{m}}\big) + \bint\limits_{\Delta^{n+1}}\di^{n+1}\underline{\lambda}(-1)^{(n+1)}\Big( T^*_{\lambda_{n+1}}Q_{\sigma(n)}(\Theta^{n+1}_K(P^q))T_{\lambda_{n+1}}\Big)
    \end{align}
...
\begin{lemma}
	...
	\begin{equation}
		\abs{\eva{\Omega, E_m(P^q) \Omega}} \leq \; C^q_m(\ell,\ell_1)\left( k_{F}^{-\frac{3}{2}} \Xi^\half
		+ k_{F}^{-1}\Xi^{\frac{3}{4}} \right)e(q)^{-1}+ (\sigma(m)-1)C'^{q}_m k_{F}^{-1} e(q)^{-1} \Xi \label{eq:finalEmest}
	\end{equation}
where
\begin{align}
	C^q_m&= \frac{C}{m!}\,  \Bigg(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^m\Bigg)\Bigg( \sum\limits_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1) \Bigg)e(q)^{-1}\nonumber\\
	\tilde{C}^q_m&= \frac{C}{m!}\,  \Bigg(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^{m+1}+\hat{V}(\ell)^m\Bigg)\Bigg( \sum\limits_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1) \Bigg)e(q)^{-1}\,.\nonumber
\end{align}
\end{lemma}
\begin{proof}
...
\begin{equation}
		\abs{\eva{\Omega, E_m(P^q) \Omega}} \leq \; \tilde{C}^q_m k_{F}^{-\half}\Xi + \tilde{C}^q_m(\ell,\ell_1)k_F^{-\frac{5}{2}} \label{eq:finalfinalEmest}
\end{equation}
...
\begin{lemma}[Bootstrap lemma]
	...
	\begin{equation}
		\eva{ T_{\lambda}\Omega. n_q T_{\lambda}\Omega} = n_q^b + n_q^{ex} + \mathfrak{C}^q k_F^{-\frac{3}{2}}
	\end{equation}
where, 
\begin{equation}
	\mathfrak{C}^q = \sum\limits_{m=1}^\infty \tilde{C}^q_m(\ell,\ell_1)
\end{equation}
\end{lemma}
\begin{proof}
	...
	\begin{equation}\label{eq:prebootexp}
		\eva{ T_{\lambda}\Omega. n_q T_{\lambda}\Omega} = n_q^b + n_q^{ex} + \mathfrak{C}^q k_F^{-\frac{1}{2}}\Xi
\end{equation}
where C is as defined above. Next we take the supremum over this expression, which results in
\begin{align}
	&\Xi = \sup_{q \in \Z^3}(n_q^b + n_q^{ex}) + \mathfrak{C}^q k_f^{-\frac{1}{2}}\Xi \nonumber\\
	\implies &\Xi(1 - \mathfrak{C}^qk_F^{-\frac{1}{2}}) = \sup_{q \in \Z^3}(n_q^b + n_q^{ex})
\end{align}
We know that the leading order terms scale as $k_F^{-2}$, then we have 
\begin{equation}
	\Xi = \frac{k_F^{-2}}{(1 - \mathfrak{C}^qk_F^{-\frac{1}{2}})}
\end{equation}
For sufficiently large $k_F$ (or sufficiently large $N$), we have that $\Xi\leq k_F^{-1}$. Next we use this bound again in \eqref{eq:prebootexp} to get the result.
\end{proof}
