...
\begin{lemma}[$E_{Q_1}(\Theta^m_{K}(P^q))$]\label{lem:finEQ1est}
	For any $\psi \in \HH_N$, we have
	\begin{align}
		\abs{\eva{\psi, E_{Q_1}\!\left(\Theta^m_K(P^q)\right)  \psi}} \leq \; &C\,  k_{F}^{-\frac{3}{2}} e(q)^{-1} \Xi^\half \Bigg(\sum\limits_{\ell \in \Z^3_*} \hat{V}(\ell)^m\Bigg)\Bigg( \sum\limits_{\ell_1 \in \Z^3_*} \hat{V}(\ell_1) \Bigg) \left(\norm { (\NN+1)^{\frac{3}{2}} \psi } + \norm { (\NN+1)^{\frac{1}{2}} \psi } \right)\nonumber\\
