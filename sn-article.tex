...
\begin{align}
	...
	&\leq 
		...
		\norm{(\NN+5)^\alpha \psi} \nonumber\\
	&\leq \Xi^{\frac 12}
		\left( \sum\limits_{\ell \in \Z^3_* \setminus S} \norm{K^m(\ell)}_{\max,2}^2 \right)^{\frac 12}
	...
	&\leq C k_{\F}^{-1} \Xi^{\frac 12}
		...
		\norm{(\NN+5) \psi} \nonumber\\
\end{align}
where we fixed $ \alpha = 1 $.
\textcolor{red}{[SL: I do not think that the conversion of $ \norm{K(\ell_1)}_{\max,2}^2 $ is legitimate, since the $ r $ appears in both indices of $ K(\ell_1) $. But one can establish an analogous bound for $ \sum_r |K(\ell_1)_{r,r+s}|^2 $.]}
On the other hand
\begin{align}
	&\abs{\sum_{\ell \in S} \I_\ell } \nonumber\\
