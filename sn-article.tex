...
\geometry{reversemp,
    ...
    bindingoffset=0mm,
    footskip=10.13mm}
\usepackage{pgfplots, subcaption, floatrow, footnote, adjustbox,float,fancyvrb, todonotes, colonequals}
...
\begin{equation}
        \bint\limits_{\Delta^{m}_1}\di^m\underline{\lambda} = \bint\limits_{0}^{1}\bint\limits_0^\lambda \bint\limits_0^{\lambda_1} \cdots \bint\limits_0^{\lambda_{m-1}}\di\lambda \di\lambda_1\cdots \di\lambda_m\quad, 
    \end{equation}
with 
\begin{equation}
        \bint\limits_{\Delta^{m}_\lambda}\di^m\underline{\lambda} = \bint\limits_0^\lambda \bint\limits_0^{\lambda_1} \cdots \bint\limits_0^{\lambda_{m-1}}\di\lambda_1\cdots \di\lambda_m\quad, 
    \end{equation}
while following
\begin{equation}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &= \left(n_q+n_{-q}\right) + \sum\limits_{m=1}^{n} \frac{(-1)^{m}}{(m)!}Q_{\sigma(m-1)}(\Theta^{m}_K\big(P^q\big) +\sum\limits_{m=1}^{\floor{(n+1)/2}}\frac{1}{(2m)! }\sum\limits_{\ell\in \Z^3_*}\mathds{1}_{L_\ell}(q)
        \Theta^{(2m)}_K\big(P^q\big)_{q,q}\nonumber\\
        &-\sum\limits_{m=1}^{n} \bint\limits_{\Delta ^m_1}\di^m\underline{\lambda} \big(T^*_{\lambda_{m}}E_{Q_{\sigma(m-1)}} \Theta^{m}_K\big(P^q\big)T_{\lambda_{m}}\big) + \bint\limits_{\Delta^{n}_{1}}\di^n\underline{\lambda}(-1)^{(n+1)}\Big( T^*_{\lambda_n}(Q_{\sigma(n)}(\Theta^{n+1}_K\big(P^q\big)T_{\lambda_n}\Big)
    \end{align}
...
For $q \in B^c_{\mathrm{F}}$ and the vacuum state $\Omega \in \HH_N$, we have
    \begin{align}
    \eva{\Omega, T_1^*\half\left(n_q+n_{-q}\right)T_1\Omega} &= \half\sum\limits_{\ell\in \Z^3_*}\mathds{1}_{L_\ell}(q) \sum\limits_{m=1}^{\floor{(n+1)/2}} \frac{\Theta^{(2m)}_K \big(P^q\big)}{(2m)!} - \half\eva{\Omega,\sum\limits_{m=1}^{n} E_m(A)\Omega}\nonumber\\
    &-\half\bint\limits_{\Delta^{n}_1}\di^n\underline{\lambda} (-1)^n\eva{\Omega,\Big( T^*_{\lambda_n}Q_{\sigma(n)}(\Theta^{n+1}_{K}(P^q)T_{\lambda_n}\Big)\Omega}
    \end{align}
\end{proposition}
\begin{proof}
...
\textcolor{red}{Put it in the introduction}    
\end{lemma}
...
\begin{proof}
    ...
    \begin{align}
        \norm{\sum\limits_{p\in L_k}f(p)b_p(k)\psi}^2 &= \eva{\sum\limits_{p\in L_k}f(p)b_p(k)\psi,\sum\limits_{q\in L_k}f(q)b_q(k)\psi}\nonumber\\
        &= \sum\limits_{p,q \in L_k} \overline{f(p)}f(q) \eva{\psi,b^*_p(k)b_q(k)\psi} \nonumber\\
        &= \sum\limits_{p,q \in L_k} \overline{f(p)}f(q) \eva{\psi,a^*_{p} a^*_{p-k}a_{q-k} a_{q})\psi}.  
    \end{align}
    We again use $a^*_{p-k}a_{q-k} \leq \mathds{1}$ to get
    \begin{equation}
        \leq \sum\limits_{p,q \in L_k} \overline{f(p)}f(q) \eva{\psi,a^*_{p} a_{q} \psi} = \norm{\sum\limits_{p \in L_k} f(p)a_{q}\psi}^2,
    \end{equation}
    and use the Cauchy-Schwarz inequality to arrive at 
    \begin{align}
    ...
    \end{align}
    For the next inequality, we use Lemma \ref{lem:paircomm} and estimate (\ref{eq:estb}). We begin with
    \begin{align}
        \norm{\sum\limits_{p\in L_k}f(p)b^*_p(k)\psi}^2 &= \eva{\sum\limits_{p\in L_k}f(p)b^*_p(k)\psi,\sum\limits_{q\in L_k}f(q)b^*_q(q)\psi}\nonumber\\
        &= \sum\limits_{p,q \in L_k} \overline{f(p)}f(q)\left( \eva{\psi,b^*_p(k)b_q(q)\psi} + \eva{\psi, \left[ b_p(k),b^*_q(q)\right]\psi}\right) \nonumber\\
        &=\sum\limits_{p,q \in L_k} \overline{f(p)}f(q)\left( \eva{\psi,b^*_p(k)b_q(q)\psi} + \eva{\psi, \left( \delta_{p,q} + \epsilon_{p,q} (k,k) \right)\psi}\right)
    \end{align}
    ...
    \begin{align}
        ...
        &=  \norm{\sum\limits_{p\in L_k}f(p)b_p(k)\psi}^2 + \norm{\sum\limits_{p\in L_k}f(p)\psi}^2\nonumber\\
...
\begin{lemma}[Gr\"onwall Bound]\label{lem:gronNest}
    ...
    \end{equation}
    for all $\Psi \in \HH_N$, where $C = \mathrm{exp}(4\sum\limits_{l\in \Z^3_*}\norm{K(\ell)}_{\mathrm{HS}})$
\end{lemma}
\begin{proof}
    We start with taking a derivative of the expectation of the RHS of the inequality above.
    \begin{align}
        ...
        &= 4 \sum\limits_{\ell \in \Z^3_*} \abs{\eva{\sum\limits_{s \in L_{\ell}}b_{-s} T_\lambda\Psi,\sum\limits_{r,s \in L_{\ell}}K(\ell)_{r,s} (-\ell)b^*_{r}(\ell)T_\lambda\Psi}}\label{eq:diffeva}
    \end{align}
    Then using Cauchy-Schwarz inequality and the estimates from Lemma \ref{lem:pairest}, we get
    \begin{align}
        (\ref{eq:diffeva}) &\leq 4\sum\limits_{\ell \in \Z^3_*}\sum\limits_{s \in L_{\ell}} \norm{b_{-s} T_\lambda\Psi} \norm{\sum\limits_{r \in L_{\ell}}K(\ell)_{r,s} (-\ell)b^*_{r}(\ell)T_\lambda\Psi} \nonumber\\
    ...
    \end{align}
    Then using Gr\"onwall lemma, we have
    \begin{equation}
...
\begin{equation}\label{eq:headest}
    \abs{\bint\limits_{\Delta^{m}_1}\di^n\underline{\lambda} \eva{\Omega,\Big( T^*_{\lambda_n}Q_{\sigma(n)}(\Theta^{n+1}_{K}(P^q))T_{\lambda_n}\Big)\Omega}}\leq \frac{2^{n+1}}{(n+1)!} \norm{K(\ell)}^{n+1}_{\mathrm{HS}} \,C \, \eva{\Omega,\Big(\NN+1\Big)\Omega} 
\end{equation}
\end{proposition}
\begin{proof}
    We first look at the case $n$ even.
    We begin with the R.H.S. of the above expression and use the the estimate from Lemma \ref{lem:estQ2} to get
    \begin{equation}
      \mathrm{R.H.S.\,of\,} (\ref{eq:headest})   
        \leq  \abs{\bint\limits_{\Delta^{n}_1}\di^n\underline{\lambda} \norm{\Theta^{n+1}_{K}(P^q)}_{\mathrm{HS}} \eva{\Omega,\Big( T^*_{\lambda_n}(\NN +1) T_{\lambda_n}\Big)\Omega}} 
    \end{equation}
    Then using Lemma \ref{lem:multicommest} we get
    \begin{equation}
        \leq  \abs{\bint\limits_{\Delta^{n}_1}\di^n\underline{\lambda} 2^{n+1}\norm{K(\ell)}^{n+1}_{\mathrm{op}}\norm{(P^q)}_{\mathrm{HS}} \eva{\Omega,\Big( T^*_{\lambda_n}(\NN +1) T_{\lambda_n}\Big)\Omega}}
    \end{equation}
    Here we observe that $\norm{(P^q)}_{\mathrm{HS}} = 1$ and $\norm{K(\ell)}_{\mathrm{op}}\leq \norm{K(\ell)}_{\mathrm{HS}}$, and then we use the Gr\"onwall estimate from Lemma \ref{lem:gronNest} to have
    \begin{align}
    &\leq \abs{\bint\limits_{\Delta^{n}_1} \di^n \underline{\lambda}2^{n+1} \norm{K(\ell)}^{n+1}_{\mathrm{op}} \,C \, \eva{\Omega,\Big(\NN+1\Big)\Omega}}\nonumber\\
    &= \frac{2^{n+1}}{(n+1)!} \norm{K(\ell)}^{n+1}_{\mathrm{op}} \,C \, \eva{\Omega,\Big(\NN+1\Big)\Omega}\nonumber
    \end{align}
    And we know $\norm{K(k)}_{\mathrm{op}}\leq \norm{K(k)}_{\mathrm{HS}}$, then 
    \begin{equation}
       \leq \frac{2^{n+1}}{(n+1)!} \norm{K(\ell)}^{n+1}_{\mathrm{HS}} \,C \, \eva{\Omega,\Big(\NN+1\Big)\Omega}\nonumber 
    \end{equation}
...
\begin{lemma}[The infinite $n$ limit]
    ...
    \lim\limits_{n\rightarrow\infty}\eva{\Omega, T_1^*\half\left(n_q+n_{-q}\right)T_1\Omega} = \half\sum\limits_{\ell\in \Z^3_*} \mathds{1}_{L_\ell}(q) \left(\cosh (2K(\ell)-1)\right)_{ q,q} -\half\lim\limits_{n\rightarrow\infty} \eva{\Omega,\sum\limits_{m=1}^n E_m(A)\Omega}
    \end{equation}
\end{lemma} 
\begin{proof}
...
\begin{definition}[Norms]
    For $k \in \Z^3_*$ and $A\in \ell^2(L_k)$, we have 
      \begin{equation}
...
\begin{definition}[Bootstrap Quantity]
 For $k \in \Z^3_*$ and $q\in L_k$, we have 
    \begin{align}
        \Xi_\lambda(q) &\coloneq \expval{T_\lambda\Omega, a^*_qa_qT_\lambda\Omega}\\
        \Xi_\lambda &\coloneq \sup\limits_{q \in L_k}\expval{T_\lambda\Omega, a^*_qa_qT_\lambda\Omega}.
    \end{align}
\end{definition}
...
We begin with 
\begin{equation}
    \big\{\{a^*_{r+1-\ell_1}a_{r-\ell}, b^*_{-s_1}(-\ell_1)\},b_{-s}(-\ell)\big\} = \{a^*_{r_1-\ell_1}a_{r-\ell}, b^*_{-s_1}(-\ell_1)\}b_{-s}(-\ell) +b_{-s}(-\ell)\{a^*_{r_1-\ell_1}a_{r-\ell}, b^*_{-s_1}(-\ell_1)\}.
\end{equation}
...
Then we have
\begin{align}
    &\big\{b_{-s}(-\ell),\{a^*_{r_1-\ell_1}a_{r-\ell}, b^*_{-s_1}(-\ell_1)\}\big\}\nonumber\\
...
\begin{alignat}{2}
    ...
    \begin{aligned}[t]
        &\Big( 2a^*_{r-\ell_1}b^*_{s}(\ell) b^*_{-s_1}(-\ell_1)a_{r-\ell} \\ &+ b^*_{s}(\ell) a^*_{r-\ell_1}[b_{-s_1}(-\ell_1),a^*_{r-\ell}]^*\Big) + \mathrm{h.c.}
    \end{aligned}\nonumber\\
    &\quad +\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r\in (L_{\ell}-\ell) \cap (L_{\ell_1}-\ell_1)\\ s \in (L_{\ell}-\ell),s_1\in (L_{\ell_1}-\ell_1)}}\!\!\!\!\!\!\!\!\!\begin{aligned}[t] A(\ell)_{r+\ell,s+\ell}&K(\ell_1)_{r+\ell_1,s_1+\ell_1}
    \Big( 2a^*_{r+\ell_1}b^*_{s+\ell}(\ell) b^*_{-s_1-\ell_1}(-\ell_1) a_{r+\ell}\nonumber \\ &+ b^*_{s+\ell}(\ell) a^*_{r+\ell_1}[b_{-s_1-\ell_1}(-\ell_1),a^*_{r+\ell}]^*\Big) + \mathrm{h.c.}
    \end{aligned}\\
\end{alignat}
and
\begin{alignat}{2}
    ...
    &+ \big\{ [b_{-s}(-\ell),a^*_{r-\ell_1}],[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}    
    \end{aligned}\\
    &\quad -\!\!\!\sum\limits_{\ell,\ell_1 \in \Z^3_*}\begin{aligned}[t]&\sum\limits_{\substack{r\in (L_{\ell}-\ell) \cap (L_{\ell_1}-\ell_1)\\ s \in (L_{\ell}-\ell),s_1\in (L_{\ell_1}-\ell_1)}}\!\!\!\!\!\!\begin{aligned}[t] A(\ell)_{r+\ell,s+\ell} K(\ell_1)_{r+\ell_1,s_1+\ell_1} &\Big(4a^*_{r+\ell_1}b^*_{-s_1-\ell_1}(-\ell_1)b_{-s-\ell}(-\ell)a_{r+\ell} \nonumber\\ 
    &+ 2a^*_{r+\ell_1}[b_{-s-\ell}(-\ell),b^*_{-s_1-\ell_1}(-\ell_1)]a_{r+\ell}\nonumber\\
    \end{aligned}\\
    &+ a^*_{r+\ell_1}[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*b_{-s-\ell}(-\ell)+ 2a^*_{r+\ell_1}\big[b_{-s-\ell}(-\ell),[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big] \nonumber\\
    &+ 2b^*_{-s_1-\ell_1}(-\ell_1)[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]a_{r+\ell} +2\big[[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]^*,b_{-s_1-\ell_1}(-\ell_1)\big]^*a_{r+\ell} \nonumber\\
    &- [a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}] + \big\{ [b_{-s-\ell}(-\ell),a^*_{r+\ell_1}],[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}    
    \end{aligned}\\
...
\begin{equation}
    \Theta^m_K(P^q)(\ell)_{r,s} = \left(K^m\cdot P^q\right)_{r,s} +\left(\sum\limits_{j=1}^{m-1} K^{m-j}\cdot P^q\cdot K^{j}\right)_{r,s} +\left( P^q\cdot K^m\right)_{r,s} 
\end{equation}
...
\begin{alignat}{2}
    &E_{Q_1}(\Theta^m_K(P^q)) =\nonumber\\ 
    &-{\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r \in             L_{\ell}\cap  L_{\ell_1}\\s_1\in L_{\ell_1}}}\!\!\!\mathds{1}_{L_\ell}      (q) K^{m}_{r,q}(\ell)K_{r,s_1}(\ell_1)\begin{aligned}[t]
        &\Big( 2a^*_{r-\ell_1}b^*_{q}(\ell) b^*_{-s_1}(-\ell_1)a_{r-\ell} \\ &+ b^*_{q}(\ell) a^*_{r-\ell_1}[b_{-s_1}(-\ell_1),a^*_{r-\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber}\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r \in              (L_{\ell}-\ell) \cap  (L_{\ell_1}-\ell_1)\\s_1\in (L_{\ell_1}-              \ell_1)}} \!\!\!\!\! \mathds{1}_{L_\ell}(q)                                 \begin{aligned}[t]K^{m}_{r+\ell,q}(\ell)
        K_{r+\ell_1,s_1+\ell_1}(\ell_1) &\Big( 2a^*_{r+\ell_1}b^*_{q}(\ell) b^*_{-s_1-\ell_1}(-\ell_1) a_{r+\ell}\nonumber \\ &+ b^*_{q}(\ell) a^*_{r+\ell_1}[b_{-s_1-\ell_1}(-\ell_1),a^*_{r+\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\!\!
        \sum\limits_{\substack{r\in L_{\ell}\cap L_{\ell_1}\\s \in L_{\ell}, s_1\in L_{\ell_1}}}\!\!\!\!\!\mathds{1}_{L_\ell}(q)\bigg(\sum_{j=1}^{m-1}K^{m-j}_{r,q}(\ell)K^{j}_{q,s}(\ell)\bigg)K_{r,s_1}(\ell_1)\begin{aligned}[t]&\Big( 2a^*_{r-\ell_1}b^*_{s}(\ell) b^*_{-s_1}(-\ell_1)a_{r-\ell} \\ &+ b^*_{s}(\ell) a^*_{r-\ell_1}[b_{-s_1}(-\ell_1),a^*_{r-\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\!\!
        \sum\limits_{\substack{r \in (L_{\ell}-\ell) \cap  (L_{\ell_1}-\ell_1)\\s\in (L_{\ell}-\ell),s_1\in (L_{\ell_1}-\ell_1)}}\!\!\begin{aligned}[t] \mathds{1}_{L_\ell}(q)\bigg(\sum_{j=1}^{m-1}&K^{m-j}_{r+\ell,q}(\ell)K^{j}_{q,s+\ell}(\ell)\bigg) 
        K_{r+\ell_1,s_1+\ell_1}(\ell_1)\times\\ \times&\Big( 2a^*_{r+\ell_1}b^*_{s+\ell}(\ell) b^*_{-s_1-\ell_1}(-\ell_1) a_{r+\ell}\nonumber \\ &+ b^*_{s+\ell}(\ell) a^*_{r+\ell_1}[b_{-s_1-\ell_1}(-\ell_1),a^*_{r+\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &- \sum\limits_{\ell, \ell_1\in \Z^3_*}\!\sum\limits_{\substack{s \in           L_{\ell}\\s_1\in L_{\ell_1}}}\!\!\!\!                                       \mathds{1}_{L_\ell}(q) \mathds{1}_{L_{\ell_1}}\!(q) K^{m}_{q,s}(\ell)       K_{q,s_1} (\ell_1)\begin{aligned}[t]  
        &\Big( 2a^*_{q-\ell_1}b^*_{s}(\ell) b^*_{-s_1}(-\ell_1)a_{q-\ell} \\ &+ b^*_{s}(\ell) a^*_{q-\ell_1}[b_{-s_1}(-\ell_1),a^*_{q-\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{s \in              (L_{\ell}-\ell) \\ s_1 \in (L_{\ell_1}-\ell_1)}} \!\!\!\!\!                 \begin{aligned}[t]  \mathds{1}_{(L_\ell-\ell)}(q)                           \mathds{1}_{(L_{\ell_1}-\ell_1)}(q&) K^{m}_{q+\ell,s+\ell}(\ell)            K_{q+\ell_1,s_1+\ell_1}(\ell_1)
        \times \\ \times &\Big( 2a^*_{q+\ell_1}b^*_{s+\ell}(\ell) b^*_{-s_1-\ell_1}(-\ell_1) a_{q+\ell}\nonumber \\ &+ b^*_{s+\ell}(\ell) a^*_{q+\ell_1}[b_{-s_1-\ell_1}(-\ell_1),a^*_{q+\ell}]^*\Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &+ (q\rightarrow-q)\nonumber\\
    &\quad\equalscolon \sum\limits_{i=1}^2 (\mathrm{I})_i +(\mathrm{II})_i +(\mathrm{III})_i + (\mathrm{IV})_i +(\mathrm{V})_i +(\mathrm{VI})_i\nonumber.
\end{alignat}
\textcolor{red}{should I push the non overlapping momenta back?}\\
and\newpage
\begin{alignat}{2}
    ...
    &-{\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{r \in             L_{\ell}\cap  L_{\ell_1}\\s_1\in L_{\ell_1}}}\!\!\!\begin{aligned}[t]
        \mathds{1}_{L_\ell}&(q) K^{m}_{r,q}(\ell)K_{r,s_1}(\ell_1)\Big(4a^*_{r-\ell_1}b^*_{-s_1}(-\ell_1)b_{-q}(-\ell)a_{r-\ell} \nonumber\\ 
        &+ 2a^*_{r-\ell_1}[b_{-q}(-\ell),b^*_{-s_1}(-\ell_1)]a_{r-\ell}+ a^*_{r-\ell_1}[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*b_{-q}(-\ell)\nonumber\\
        &+ 2a^*_{r-\ell_1}\big[b_{-q}(-\ell),[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*\big] + 2b^*_{-s_1}(-\ell_1)[b_{-q}(-\ell),a^*_{r-\ell_1}]a_{r-\ell} \nonumber\\
        &+2\big[ [ b_{-q}(-\ell) , a^*_{r-\ell_1}]^* , b_{-s_1}(-\ell_1) \big]^* a_{r-\ell} - [a^*_{r-\ell} , b_{-s_1}(-\ell_1) ]^* [ b_{-q}(-\ell) , a^*_{r-\ell_1} ] \nonumber\\
        &+ \big\{ [b_{-q}(-\ell),a^*_{r-\ell_1}],[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.} 
        \end{aligned}\nonumber}\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\begin{aligned}[t]
        &\sum\limits_{\substack{r \in (L_{\ell}-\ell) \cap  (L_{\ell_1}-\ell_1)\\s_1\in (L_{\ell_1}-              \ell_1)}}\!\!\!\!\!\begin{aligned}[t]\mathds{1}_{L_\ell}(q)                 K^{m}_{r+\ell,q}(\ell)
        K_{r+\ell_1,s_1+\ell_1}(\ell_1)&\Big(4a^*_{r+\ell_1}b^*_{-s_1-\ell_1}(-\ell_1)b_{-q-\ell}(-\ell)a_{r+\ell} \nonumber\\ 
        &+ 2a^*_{r+\ell_1}[b_{-q-\ell}(-\ell),b^*_{-s_1-\ell_1}(-\ell_1)]a_{r+\ell}\nonumber\\
        \end{aligned}\\
        &+ a^*_{r+\ell_1}[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*b_{-q-\ell}(-\ell)+ 2a^*_{r+\ell_1}\big[b_{-q-\ell}(-\ell),[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big] \nonumber\\
        &+ 2b^*_{-s_1-\ell_1}(-\ell_1)[b_{-q-\ell}(-\ell),a^*_{r+\ell_1}]a_{r+\ell} +2\big[[b_{-q-\ell}(-\ell),a^*_{r+\ell_1}]^*,b_{-s_1-\ell_1}(-\ell_1)\big]^*a_{r+\ell} \nonumber\\
        &- [a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*[b_{-q-\ell}(-\ell),a^*_{r+\ell_1}] + \big\{ [b_{-q-\ell}(-\ell),a^*_{r+\ell_1}],[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\!\!
        \sum\limits_{\substack{r\in L_{\ell}\cap L_{\ell_1}\\s \in L_{\ell}, s_1\in L_{\ell_1}}}\!\!\!\!\!\begin{aligned}[t]\mathds{1}_{L_\ell}
        &(q)\bigg(\sum_{j=1}^{m-1}K^{m-j}_{r,q}(\ell)K^{j}_{q,s}(\ell)\bigg)K_{r,s_1}(\ell_1)\Big(4a^*_{r-\ell_1}b^*_{-s_1}(-\ell_1)b_{-s}(-\ell)a_{r-\ell} \nonumber\\ 
    &+ 2a^*_{r-\ell_1}[b_{-s}(-\ell),b^*_{-s_1}(-\ell_1)]a_{r-\ell}+ a^*_{r-\ell_1}[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*b_{-s}(-\ell)\nonumber\\
    &+ 2a^*_{r-\ell_1}\big[b_{-s}(-\ell),[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*\big] + 2b^*_{-s_1}(-\ell_1)[b_{-s}(-\ell),a^*_{r-\ell_1}]a_{r-\ell} \nonumber\\
    &+2\big[[b_{-s}(-\ell),a^*_{r-\ell_1}]^*,b_{-s_1}(-\ell_1)\big]^*a_{r-\ell}- [a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*[b_{-s}(-\ell),a^*_{r-\ell_1}] \nonumber\\
    &+ \big\{ [b_{-s}(-\ell),a^*_{r-\ell_1}],[a^*_{r-\ell}, b_{-s_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}
    ...
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\!\!\begin{aligned}[t] &
        \sum\limits_{\substack{r \in (L_{\ell}-\ell) \cap  (L_{\ell_1}-\ell_1)\\s\in (L_{\ell}-\ell),s_1\in (L_{\ell_1}-\ell_1)}}\!\!\begin{aligned}[t]\mathds{1}_{L_\ell}&(q)\bigg(\sum_{j=1}^{m-1}K^{m-j}_{r+\ell,q}(\ell)K^{j}_{q,s+\ell}(\ell)\bigg) 
        K_{r+\ell_1,s_1+\ell_1}(\ell_1)\times\\\times &\Big(4a^*_{r+\ell_1}b^*_{-s_1-\ell_1}(-\ell_1)b_{-s-\ell}(-\ell)a_{r+\ell}  
    + 2a^*_{r+\ell_1}[b_{-s-\ell}(-\ell),b^*_{-s_1-\ell_1}(-\ell_1)]a_{r+\ell}\nonumber\\
    \end{aligned}\\
    &+ a^*_{r+\ell_1}[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*b_{-s-\ell}(-\ell)+ 2a^*_{r+\ell_1}\big[b_{-s-\ell}(-\ell),[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big] \nonumber\\
    &+ 2b^*_{-s_1-\ell_1}(-\ell_1)[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]a_{r+\ell} +2\big[[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]^*,b_{-s_1-\ell_1}(-\ell_1)\big]^*a_{r+\ell} \nonumber\\
    &- [a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}] + \big\{ [b_{-s-\ell}(-\ell),a^*_{r+\ell_1}],[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &- \sum\limits_{\ell, \ell_1\in \Z^3_*}\!\sum\limits_{\substack{s \in           L_{\ell}\\s_1\in L_{\ell_1}}}\!\!\!\!\begin{aligned}[t]                     \mathds{1}_{L_\ell}(q) \mathds{1}_{L_{\ell_1}}\!&(q) K^{m}_{q,s}(\ell)       K_{q,s_1} (\ell_1)\Big(4a^*_{q-\ell_1}b^*_{-s_1}(-\ell_1)b_{-s}(-\ell)a_{q-\ell} \nonumber\\ 
    &+ 2a^*_{q-\ell_1}[b_{-s}(-\ell),b^*_{-s_1}(-\ell_1)]a_{q-\ell}+ a^*_{q-\ell_1}[a^*_{q-\ell}, b_{-s_1}(-\ell_1)]^*b_{-s}(-\ell)\nonumber\\
    &+ 2a^*_{q-\ell_1}\big[b_{-s}(-\ell),[a^*_{q-\ell}, b_{-s_1}(-\ell_1)]^*\big] + 2b^*_{-s_1}(-\ell_1)[b_{-s}(-\ell),a^*_{q-\ell_1}]a_{q-\ell} \nonumber\\
    &+2\big[[b_{-s}(-\ell),a^*_{q-\ell_1}]^*,b_{-s_1}(-\ell_1)\big]^*a_{q-\ell}- [a^*_{q-\ell}, b_{-s_1}(-\ell_1)]^*[b_{-s}(-\ell),a^*_{q-\ell_1}] \nonumber\\
    &+ \big\{ [b_{-s}(-\ell),a^*_{q-\ell_1}],[a^*_{q-\ell}, b_{-s_1}(-\ell_1)]^*\big\} \Big) + \mathrm{h.c.}
        \end{aligned}\nonumber\\
    &-\sum\limits_{\ell, \ell_1\in \Z^3_*}\sum\limits_{\substack{s \in              (L_{\ell}-\ell) \\ s_1 \in (L_{\ell_1}-\ell_1)}} \!\!\!\!\!                 \begin{aligned}[t]  &\mathds{1}_{(L_\ell-\ell)}(q)                           \mathds{1}_{(L_{\ell_1}-\ell_1)}(q)\begin{aligned}[t] K^{m}_{q+\ell,s+\ell}(\ell)           & K_{q+\ell_1,s_1+\ell_1}(\ell_1)
        \Big(4a^*_{r+\ell_1}b^*_{-s_1-\ell_1}(-\ell_1)b_{-s-\ell}(-\ell)a_{r+\ell} \nonumber\\ 
    &+ 2a^*_{r+\ell_1}[b_{-s-\ell}(-\ell),b^*_{-s_1-\ell_1}(-\ell_1)]a_{r+\ell}\nonumber\\
    \end{aligned}\\
    &+ a^*_{r+\ell_1}[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*b_{-s-\ell}(-\ell)+ 2a^*_{r+\ell_1}\big[b_{-s-\ell}(-\ell),[a^*_{r+\ell}, b_{-s_1-\ell_1}(-\ell_1)]^*\big] \nonumber\\
    &+ 2b^*_{-s_1-\ell_1}(-\ell_1)[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]a_{r+\ell} +2\big[[b_{-s-\ell}(-\ell),a^*_{r+\ell_1}]^*,b_{-s_1-\ell_1}(-\ell_1)\big]^*a_{r+\ell} \nonumber\\
    ...
    &+ (q\rightarrow-q)\nonumber\\
    &-\sum\limits_{\ell \in \Z^3_*}\sum\limits_{r,s \in L_{\ell}}\Theta^{m+1}_K(P^q)_{r,s}\epsilon_{r,s}(\ell,\ell)\nonumber\\
    &\equalscolon \sum\limits_{i=1}^{20}(\mathrm{A})_i +(\mathrm{B})_i +(\mathrm{C})_i +(\mathrm{D})\nonumber.
 \end{alignat}
with all the terms $(\mathrm{I})_i,(\mathrm{II})_i,(\mathrm{III})_i,(\mathrm{A})_i,(\mathrm{B})_i,(\mathrm{C})_i$ have the contributions from both $q$ and $-q$. As evident from the normal ordering, the first term (the leading error contribution), i.e.,  $(\mathrm{I})_1,(\mathrm{II})_1,(\mathrm{III})_1,(\mathrm{A})_1,(\mathrm{B})_1,(\mathrm{C})_1$ coming from $E_{Q_1}$ and $E_{Q_2}$ have six fermionic operators each.


\begin{lemma}
For any $\psi \in \HH_N$, we have
\begin{align}
     &\abs{\eva{\psi,\sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{r \in L_\ell \cap L_{\ell_1}}a^*_{r-\ell} 
     \big(K^{m}_{r,q}(\ell)b^*_{q}(\ell)\big) \bigg( \sum\limits_{s_1 \in L_{\ell_1}} K(\ell_1)_{r,s_1}b^*_{-s_1}(-\ell_1) \bigg) a_{r-\ell_1} \psi }}\nonumber\\
     &\leq  C \sup\limits_{q \in L_\ell} \norm{n_{q}^{\half} \psi} \Bigg(\sum\limits_{\ell \in \Z^3_*} \norm{K(\ell)}_{\mathrm{max}}\Bigg)\Bigg( \sum\limits_{\ell_1 \in \Z^3_*}\norm{K(\ell_1)}_{\mathrm{max},2} \Bigg)  \norm { (\NN+1)^{\frac{3}{2}} \psi } \label{eq:headboserra}
\end{align}
\end{lemma}
\begin{proof}
We start by using resolution of the identity $I = (\NN+1)^{\alpha}(\NN+1)^{-\alpha}$ for some $\alpha \in \R$. Then use the Cauchy-Schwarz inequality and the bounds from Lemma \ref{lem:pairest} on the R.H.S. of (\ref{eq:headboserra})
\begin{align}
    &=\sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{r \in L_\ell \cap L_{\ell_1}}\abs{\eva{\big(K^{m}_{r,q}(\ell)b_{q}(\ell)\big) a_{r-\ell}(\NN+1)^{\alpha}(\NN+1)^{-\alpha} \psi, \bigg( \sum\limits_{s_1 \in L_{\ell_1}} K(\ell_1)_{r,s_1}b^*_{-s_1}(-\ell_1) \bigg) a_{r-\ell_1} \psi }}\nonumber\\
    &=\sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{r \in L_\ell \cap L_{\ell_1}}\abs{\eva{\big(K^{m}_{r,q}(\ell)b_{q}(\ell)\big) a_{r-\ell}(\NN+1)^{-\alpha} \psi, \bigg( \sum\limits_{s_1 \in L_{\ell_1}} K(\ell_1)_{r,s_1}b^*_{-s_1}(-\ell_1) \bigg) a_{r-\ell_1}(\NN+5)^{\alpha} \psi }}\nonumber\\
    &\leq \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{r \in L_\ell \cap L_{\ell_1}}\norm{\big(K^{m}_{r,q}(\ell)b_{q}(\ell)\big) a_{r-\ell} (\NN+1)^{-\alpha}\psi}\norm {\bigg( \sum\limits_{s_1 \in L_{\ell_1}} K(\ell_1)_{r,s_1}b^*_{-s_1}(-\ell_1) \bigg) a_{r-\ell_1} (\NN+5)^{\alpha}\psi }\nonumber\\
    &\leq \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{r \in L_\ell} \norm{\big(K^{m}_{r,q}(\ell)b_{q}(\ell)\big) a_{r-\ell} (\NN+1)^{-\alpha} \psi}\bigg( \sum\limits_{s_1 \in L_{\ell_1}} \abs{K(\ell_1)_{r,s_1}}^2 \bigg)^\half \norm {\left(\NN+1\right)^\half a_{r-\ell_1} (\NN+5)^{\alpha} \psi }\nonumber\\
    &\leq \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \norm{K(\ell_1)}_{\mathrm{max},2} \sum\limits_{r \in L_\ell} \norm{\big(K^{m}_{r,q}(\ell)b_{q}(\ell)\big) a_{r-\ell} (\NN+1)^{-\alpha} \psi} \norm { a_{r-\ell_1}\NN^\half (\NN+5)^{\alpha} \psi }\nonumber\\
    &\leq \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \norm{K(\ell)}_{\mathrm{max}}\norm{K(\ell_1)}_{\mathrm{max},2} \sum\limits_{r \in L_\ell} \norm{\big(b_{q}(\ell)\big) a_{r-\ell} (\NN+1)^{-\alpha} \psi} \norm { a_{r-\ell_1}\NN^\half (\NN+5)^{\alpha} \psi }\nonumber\\
    &\leq \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \norm{K(\ell)}_{\mathrm{max}}\norm{K(\ell_1)}_{\mathrm{max},2} \Bigg(\sum\limits_{r \in L_\ell} \norm{ a_{r-\ell}b_{q}(\ell) (\NN+1)^{-\alpha} \psi}^2\Bigg)^\half\Bigg( \sum\limits_{r \in L_\ell} \norm { a_{r-\ell_1}\NN^\half (\NN+5)^{\alpha} \psi }^2\Bigg)^\half\nonumber\\
    \end{align}
...
Next, we consider the (B)/(II) term

\begin{lemma}
For any $\psi \in \HH_N$, we have
\begin{align}
     &\abs{\eva{\psi,\sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{\substack{r,s \in L_{\ell}\\r_1,s_1\in L_{\ell_1}}}  a^*_{r-\ell}
     \Bigg(\sum_{j=1}^{m-1}K^{m-j}(\ell)_{r,q}K^{j}(\ell)_{q,s}b^*_{s}(\ell)\Bigg)  K(\ell_1)_{r_1,s_1}b^*_{-s_1}(-\ell_1) a_{r_1-\ell_1} \psi }}\nonumber\\
...
\end{lemma}
\begin{proof}
    We begin with the R.H.S. of \eqref{eq:headboserrb}, which is 
    \begin{align}
        &= \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{\substack{r,s \in L_{\ell}\\r_1,s_1\in L_{\ell_1}}} \sum_{j=1}^{m-1} \abs{\eva{\psi,  a^*_{r-\ell} \Bigg(K^{m-j}(\ell)_{r,q}K^{j}(\ell)_{q,s}b^*_{s}(\ell)\Bigg)  K(\ell_1)_{r_1,s_1}b^*_{-s_1}(-\ell_1) a_{r_1-\ell_1} \psi }}\nonumber\\
        &= \sum\limits_{\ell,\ell_1 \in \Z^3_*} \mathds{1}_{L_\ell}(q) \sum\limits_{\substack{r,s \in L_{\ell}\\r_1,s_1\in L_{\ell_1}}} \sum_{j=1}^{m-1} \abs{\eva{  \Big(K^{m-j}(\ell)_{r,q}K^{j}(\ell)_{q,s}b_{s}(\ell)\Big) a_{r-\ell}(\NN+1)^{\alpha}(\NN+1)^{-\alpha}\psi,  K(\ell_1)_{r_1,s_1}b^*_{-s_1}(-\ell_1) a_{r_1-\ell_1} \psi }}\nonumber\\
