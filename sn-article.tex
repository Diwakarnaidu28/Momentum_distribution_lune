...
For $k,\ell \in \Z^3_*$ and, $p \in L_{k}$ and $q\in L_{\ell}$, we have
   ...
   with $\epsilon_{p,q}(k,k) = \epsilon^*_{q,p}(k,k) $ and $\epsilon_{p,p}(k,k)\leq 0$ \end{lemma}
...
\begin{align}
    T_1^*\left(n_q+n_{-q}\right)T_1 &=\left(n_q+n_{-q}\right)+\sum\limits_{\ell\in \Z^3_*}\operatorname{Tr}\left(\cosh(2K(\ell)) -\mathds{1}\right) + \sum\limits_{m=1}^n E_m(P^q)\\&- Q_2\left(\sum\limits_{m=1}^{\floor{(n+1)/2}}\frac{\Theta^{2n-1}_{K}(P^q)} {(2n-1)!}\right)+ Q_1\left(\sum\limits_{m=1}^{\floor{n/2}}\frac{\Theta^{2m}_{K}(P^q)}{(2m)!}\right)  \nonumber\\
...
\begin{equation}
    ...
    \end{equation}
    and, $\Theta^n_{K}$, the simplex integral and $\sigma(n)$ are defined above.
\end{proposition}
\begin{remark}
    In the infinite n limit, the $Q_1$ and $Q_2$ terms converge respectively to a $\cosh$ and $\sinh$ series in their arguments
\end{remark}
\begin{proof}
    ...
    \begin{align}
        ...
        &= \left(n_q+n_{-q}\right) + \sum\limits_{m=1}^{n} \frac{(-1)^{m}}{(m)!}Q_{\sigma(m-1)}(\Theta^{m}_K\big(P^q\big) +\sum\limits_{m=1}^{\floor{(n+1)/2}}\frac{1}{(2m)!}\sum\limits_{\ell\in \Z^3_*}\sum\limits_{r\in L_\ell}\Theta^{(2m)}_K\big(P^q\big)_{r,r}\nonumber\\
    ...
    the quadratic operators are linear in their argument, we can interpret them as $\cosh$ and $\sinh$ series of $\Theta_{K}(P^q)$ operator. In the third term, again using the linearity of the sum over all momenta tranfer $\ell$ and the trace we recover the trace term. And for the fourth term, we just use the definition of $E_M(A)$. Doing these identifications we arrive at the desired operator identity.  
\end{proof}
\subsubsection{Evaluation of the expectation value}
\begin{lemma}\label{lem:evaquad}
    For the quadratic operators $Q_1(A)$ and $Q_2(B)$ for symmetric 
    $A,B:\ell^2(L_\ell)\rightarrow\ell^2(L_\ell)$, we have
    \begin{align}
...
\end{lemma}
\begin{proof}
The proof follows by the plugging in the definitions of $Q_1$ and $Q_2$ operators and observing that fact that both $Q_1$ and $Q_2$ are normal order in the fermionic creation and annihilation operators.     
\end{proof}
\begin{proposition}[Final Expectation]
    ...
    &-\bint\limits_{\Delta^{m}_\lambda}\di^n\underline{\lambda} (-1)^n\eva{\Omega,\Big( T^*_{\lambda_n}Q_{\sigma(n)}(\Theta^{n+1}_{K}(P^q)T_{\lambda_n}\Big)\Omega}\nonumber
    \end{align}
\end{proposition}
\begin{proof}
...
\begin{lemma}[Bounds on Pair Operators]\label{lem:pairest}
    ...
    \begin{equation}\label{eq:estopb}
        \sum\limits_{p \in L_k}\norm{b_p(k)\psi} \leq  \norm{\NN^\half\psi} \quad \quad \forall \psi \in \HH_N .
    \end{equation}
...
\begin{proof}
    ...
    \begin{align}
         \sum\limits_{p \in L_k}\norm{b_p(k)\psi}^2 &= \sum\limits_{p,q \in L_k} \eva{b_p(k)\psi,b_q(k)\psi}\nonumber\\
        &= \sum\limits_{p,q \in L_k} \eva{\psi,(a^*_{p} a^*_{p-k}a_{q-k} a_{q})\psi}.  
    \end{align}
    We use $a^*_{p-k}a_{q-k} \leq \mathds{1}$ to get 
    \begin{equation}
        \leq \sum\limits_{p,q \in L_k} \eva{\psi,(a^*_{p} a_{q})\psi}
        \leq \eva{\psi,\sum\limits_{p \in \Z^3_*}(a^*_{p} a_{q})\psi} = \eva{\psi, \NN\psi}
    \end{equation}
...
\begin{proof}
    \begin{align}
        ...
        &=2\sum\limits_{\ell\in \Z^3_*}\left|\eva{\sum\limits_{q\in L_{\ell}}b_{-q}(-\ell)\Psi,\sum\limits_{p,q\in L_{\ell}}A(\ell)_{p,q}b_p^*(\ell) \Psi}\right|\nonumber\\
        &\leq 2\sum\limits_{\ell\in \Z^3_*} \norm{\sum\limits_{q\in L_{\ell}}b_{-q}(-\ell)\Psi}
        \norm{\sum\limits_{p,q\in L_{\ell}}A(\ell)_{p,q}b^*_{p}(\ell)\Psi}    
    \end{align}
    ...
    \begin{align}
        ...
        &\leq 2\sum\limits_{\ell\in \Z^3_*}\norm{A(\ell)}_{\mathrm{HS}}\eva{\Psi,(\mathcal{N}+1) \Psi}
    \end{align}
    Hence we have the required estimate.
\end{proof}
\begin{proposition}[Gr\"onwall Bound]
    For all $\Psi \in \HH_N$
    \begin{equation}
    \left|\eva{\Psi, T^*_{\lambda}(\mathcal{N} +1)T_{\lambda} \Psi }\right|\leq e^C\eva{\Psi, (\NN+1)\Psi}    
    \end{equation}
    where $C = \mathrm{exp}(4\sum\limits_{l\in \Z^3_*}\norm{K(\ell)}_{\mathrm{HS}})$
    \textcolor{red}{assumptions to be filled in}
\end{proposition}
\begin{proof}
    We start with the taking a derivative of the RHS of the inequality above.
    \begin{align}
    ...
    \begin{align}
        ...
        &\leq 4 \sum\limits_{\ell \in \Z^3_*} \norm{K(\ell)}_{\mathrm{HS}} \eva{\Psi,T^*_\lambda(\NN+1)
        T_\lambda\Psi}
        \end{align}
...
\begin{lemma}[HS Norm bound on the K]
    \begin{equation}
    \norm{K(\ell)}_{\mathrm{HS}}\leq    
    \end{equation}
\end{lemma}
\begin{proof}
    \textcolor{red}{to be filled}
\end{proof}
\begin{lemma}[Bound on Multi anti-commutator]
    \begin{equation}
    \sum\limits_{\ell \in \Z^3_*}\norm{\Theta^n_K(A)(\ell)}_{\mathrm{HS}}\leq    \sum\limits_{\ell \in \Z^3_*} 2^{n+1}\norm{K(\ell)}_{\mathrm{op}}\norm{A(\ell)}_{\mathrm{HS}}
    \end{equation}   
\end{lemma}
\begin{proof}
    \textcolor{red}{to be filled}
...
\begin{align}
    \bint\limits_{\Delta^{m}_\lambda}\di^n\underline{\lambda} (-1)^n\eva{\Omega,\Big( T^*_{\lambda_n}Q_{\sigma(n)}(\Theta^{n+1}_{K}(P^q)T_{\lambda_n}\Big)\Omega}&=jhj
\end{align}
\end{proposition}
\begin{proof}
